# File anotasi dari `Ipynb/TRAIN_CONVLSTM_FINAL_GOOGLE_COLAB.py`.
# File ini untuk belajar/lampiran, bukan source utama aplikasi.
# Konteks laporan: Modeling, yaitu membangun dan melatih model ConvLSTM.
# Setiap komentar menjelaskan tujuan baris kode di bawahnya secara singkat.
# Catatan pembuka/penutup yang menjelaskan isi bagian kode.
"""Komentar file skripsi:
# Isi catatan penjelasan pada bagian kode ini.
Script training ConvLSTM awal sebagai pembanding sebelum pendekatan patch risk map.

# Isi catatan penjelasan pada bagian kode ini.
Konteks laporan: file ini mendukung tahapan CRISP-DM pada BAB IV,
# Isi catatan penjelasan pada bagian kode ini.
terutama data understanding, data preparation, modeling, evaluation,
# Isi catatan penjelasan pada bagian kode ini.
atau deployment sesuai fungsi teknisnya.
# Catatan pembuka/penutup yang menjelaskan isi bagian kode.
"""

# Menjaga cara baca kode Python tetap cocok dengan versi yang digunakan.
from __future__ import annotations

# Mengambil alat bantu/library yang diperlukan oleh file ini.
import argparse
# Mengambil alat bantu/library yang diperlukan oleh file ini.
import json
# Mengambil alat bantu/library yang diperlukan oleh file ini.
import re
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from dataclasses import dataclass
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from datetime import datetime
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from pathlib import Path
# Mengambil alat bantu/library yang diperlukan oleh file ini.
import shutil

# Mencoba menjalankan proses yang mungkin gagal.
try:
    # Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
    from google.colab import drive  # type: ignore
    # Melanjutkan langkah kerja pada bagian kode ini.
    drive.mount("/content/drive")
# Menangani kesalahan agar program tidak langsung berhenti.
except Exception as exc:
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print("Drive mount dilewati:", exc)

# Mengambil alat bantu/library yang diperlukan oleh file ini.
import numpy as np
# Mengambil alat bantu/library yang diperlukan oleh file ini.
import tensorflow as tf
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from PIL import Image, ImageFilter
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from tensorflow.keras import Model, Sequential
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from tensorflow.keras.callbacks import EarlyStopping, ModelCheckpoint, ReduceLROnPlateau
# Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
from tensorflow.keras.layers import (
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    BatchNormalization,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    Concatenate,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    Conv2D,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    ConvLSTM2D,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    Dropout,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    Input,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    Lambda,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    MaxPooling2D,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    TimeDistributed,
    # Menyebutkan item yang ikut dipakai pada daftar di atas.
    UpSampling2D,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
)


# Menentukan nilai tetap yang dipakai sebagai pengaturan proses.
DATE_PATTERN = re.compile(r"FIRMS_(\d{4}-\d{2}-\d{2})")
# Menentukan nilai tetap yang dipakai sebagai pengaturan proses.
DEFAULT_IMAGE_EXTENSIONS = (".jpg", ".jpeg", ".png")


# Memberi tanda khusus untuk cara kerja fungsi atau class di bawahnya.
@dataclass(frozen=True)
# Membuat wadah bernama `DatasetRecord` untuk menyimpan data atau aturan kerja.
class DatasetRecord:
    # Menjelaskan data `path` yang disimpan atau dikirim pada bagian ini.
    path: Path
    # Menjelaskan data `date` yang disimpan atau dikirim pada bagian ini.
    date: datetime


# Membuat langkah kerja bernama `build_versioned_path`.
def build_versioned_path(path: Path, label: str) -> Path:
    # Menyimpan nilai ke `timestamp` untuk dipakai pada langkah berikutnya.
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return path.with_name(f"{path.stem}_{label}_{timestamp}{path.suffix}")


# Membuat langkah kerja bernama `resolve_output_paths`.
def resolve_output_paths(output_model: Path, output_report: Path, overwrite: bool) -> tuple[Path, Path]:
    # Mengecek syarat sebelum melanjutkan proses.
    if overwrite:
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return output_model, output_report

    # Mengecek syarat sebelum melanjutkan proses.
    if not output_model.exists() and not output_report.exists():
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return output_model, output_report

    # Menyimpan nilai ke `versioned_model` untuk dipakai pada langkah berikutnya.
    versioned_model = build_versioned_path(output_model, "run")
    # Menyimpan nilai ke `versioned_report` untuk dipakai pada langkah berikutnya.
    versioned_report = build_versioned_path(output_report, "run")

    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print("Artefak lama sudah ada. Menyimpan run baru tanpa menimpa file lama.")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"Model baru: {versioned_model}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"Report baru: {versioned_report}")
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return versioned_model, versioned_report


# Membuat langkah kerja bernama `resolve_dataset_dir`.
def resolve_dataset_dir(dataset_dir_arg: str) -> Path:
    # Menyimpan nilai ke `direct_path` untuk dipakai pada langkah berikutnya.
    direct_path = Path(dataset_dir_arg).expanduser()
    # Menyimpan nilai ke `candidates` untuk dipakai pada langkah berikutnya.
    candidates = [direct_path]

    # Menyimpan nilai ke `variants` untuk dipakai pada langkah berikutnya.
    variants = [
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        Path(
            # Melanjutkan langkah kerja pada bagian kode ini.
            "/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/data/dataset_riau/Dataset_History_Fire_Hotspot_In_Riau_Province"
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        Path(
            # Melanjutkan langkah kerja pada bagian kode ini.
            "/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/data/Dataset_History_Fire_Hotspot_In_Riau_Province"
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ]

    # Mengulang proses untuk setiap data dalam daftar.
    for candidate in variants:
        # Mengecek syarat sebelum melanjutkan proses.
        if candidate not in candidates:
            # Melanjutkan langkah kerja pada bagian kode ini.
            candidates.append(candidate)

    # Mengulang proses untuk setiap data dalam daftar.
    for candidate in candidates:
        # Mengecek syarat sebelum melanjutkan proses.
        if candidate.exists():
            # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
            return candidate.resolve()

    # Menyimpan nilai ke `searched` untuk dipakai pada langkah berikutnya.
    searched = "\n".join(f"- {candidate}" for candidate in candidates)
    # Menghentikan proses dan memberi pesan kesalahan yang jelas.
    raise FileNotFoundError(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "Dataset tidak ditemukan. Path yang dicek:\n"
        # Melanjutkan langkah kerja pada bagian kode ini.
        f"{searched}\n\n"
        # Melanjutkan langkah kerja pada bagian kode ini.
        "Pastikan Google Drive sudah termount dan nama folder di Drive sesuai."
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )


# Membuat langkah kerja bernama `parse_args`.
def parse_args() -> argparse.Namespace:
    # Menyimpan nilai ke `parser` untuk dipakai pada langkah berikutnya.
    parser = argparse.ArgumentParser(
        # Menyimpan nilai ke `description` untuk dipakai pada langkah berikutnya.
        description="Latih model ConvLSTM final dari dataset hotspot harian lokal."
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--dataset-dir",
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default="/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/data/dataset_riau/Dataset_History_Fire_Hotspot_In_Riau_Province",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Folder dataset gambar harian.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--image-extensions",
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=",".join(DEFAULT_IMAGE_EXTENSIONS),
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Daftar ekstensi gambar yang dibaca, pisahkan dengan koma. Contoh: .jpg,.jpeg,.png",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--output-model",
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default="/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/artifacts/best_model_convlstm_final.keras",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Path model terbaik output.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--output-report",
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default="/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/artifacts/convlstm_training_report.json",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Path JSON ringkasan training.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument("--img-size", type=int, default=128, help="Ukuran citra grid.")
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument("--seq-length", type=int, default=7, help="Panjang sequence input.")
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument("--epochs", type=int, default=35, help="Jumlah epoch maksimum.")
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument("--batch-size", type=int, default=4, help="Batch size training.")
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--train-ratio",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=float,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=0.70,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Rasio train kronologis.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--val-ratio",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=float,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=0.15,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Rasio validation kronologis.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--threshold",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=float,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=0.3,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Threshold default evaluasi mask prediksi.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--ground-truth-threshold",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=float,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=0.05,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Threshold untuk mengubah mask density menjadi label ground-truth biner.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--dilation-kernel",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=int,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=5,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Kernel MaxFilter ganjil untuk menjaga hotspot kecil saat downsampling.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--max-pos-weight",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=float,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=50.0,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Batas atas bobot positif untuk weighted BCE.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--positive-oversample-factor",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=int,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=1,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Pengulangan tambahan untuk sample target yang mengandung hotspot.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--loss-strategy",
        # Menyimpan nilai ke `choices` untuk dipakai pada langkah berikutnya.
        choices=["wbce_dice", "wbce_tversky", "wbce_dice_context", "wbce_dice_ohem"],
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default="wbce_dice_context",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Strategi loss untuk segmentasi hotspot.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--feature-stack",
        # Menyimpan nilai ke `choices` untuk dipakai pada langkah berikutnya.
        choices=["mask", "mask_context3"],
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default="mask",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Susunan channel fitur yang diekstrak dari setiap citra harian.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--context-kernel",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=int,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=5,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Kernel konteks untuk loss dan feature stack kontekstual.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--ohem-negative-ratio",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=float,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=3.0,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Rasio jumlah negative hard pixels yang dipertahankan terhadap jumlah positive pixels pada loss OHEM.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--enable-augmentation",
        # Menyimpan nilai ke `action` untuk dipakai pada langkah berikutnya.
        action="store_true",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Aktifkan augmentasi spasial pada train set.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--search-baseline-128",
        # Menyimpan nilai ke `action` untuk dipakai pada langkah berikutnya.
        action="store_true",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Jalankan pencarian konfigurasi terkontrol di sekitar baseline terbaik 128x128.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--search-improved-128",
        # Menyimpan nilai ke `action` untuk dipakai pada langkah berikutnya.
        action="store_true",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Jalankan pencarian kandidat perbaikan model 128x128 yang tetap kompatibel dengan dataset saat ini.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--search-epochs",
        # Menyimpan nilai ke `type` untuk dipakai pada langkah berikutnya.
        type=int,
        # Menyimpan nilai ke `default` untuk dipakai pada langkah berikutnya.
        default=18,
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Jumlah epoch untuk setiap kandidat saat mode search baseline aktif.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menambahkan pilihan saat script dijalankan dari terminal.
    parser.add_argument(
        # Melanjutkan langkah kerja pada bagian kode ini.
        "--overwrite",
        # Menyimpan nilai ke `action` untuk dipakai pada langkah berikutnya.
        action="store_true",
        # Menyimpan nilai ke `help` untuk dipakai pada langkah berikutnya.
        help="Timpa model/report jika sudah ada.",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `args, _` untuk dipakai pada langkah berikutnya.
    args, _ = parser.parse_known_args()
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return args


# Membuat langkah kerja bernama `feature_stack_channels`.
def feature_stack_channels(feature_stack: str) -> int:
    # Mengecek syarat sebelum melanjutkan proses.
    if feature_stack == "mask_context3":
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return 3
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return 1


# Membuat langkah kerja bernama `parse_image_extensions`.
def parse_image_extensions(value: str) -> tuple[str, ...]:
    # Menyimpan nilai ke `extensions` untuk dipakai pada langkah berikutnya.
    extensions: list[str] = []
    # Mengulang proses untuk setiap data dalam daftar.
    for item in value.split(","):
        # Menyimpan nilai ke `ext` untuk dipakai pada langkah berikutnya.
        ext = item.strip().lower()
        # Mengecek syarat sebelum melanjutkan proses.
        if not ext:
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            continue
        # Mengecek syarat sebelum melanjutkan proses.
        if not ext.startswith("."):
            # Menyimpan nilai ke `ext` untuk dipakai pada langkah berikutnya.
            ext = f".{ext}"
        # Mengecek syarat sebelum melanjutkan proses.
        if ext not in extensions:
            # Melanjutkan langkah kerja pada bagian kode ini.
            extensions.append(ext)
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return tuple(extensions) if extensions else DEFAULT_IMAGE_EXTENSIONS


# Membuat langkah kerja bernama `_extract_hotspot_density`.
def _extract_hotspot_density(
    # Menjelaskan data `path` yang disimpan atau dikirim pada bagian ini.
    path: Path,
    # Menjelaskan data `img_size` yang disimpan atau dikirim pada bagian ini.
    img_size: int,
    # Menjelaskan data `dilation_kernel` yang disimpan atau dikirim pada bagian ini.
    dilation_kernel: int,
    # Menyimpan nilai ke `feature_stack` untuk dipakai pada langkah berikutnya.
    feature_stack: str = "mask",
    # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
    context_kernel: int = 5,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> np.ndarray:
    # Mengecek syarat sebelum melanjutkan proses.
    if dilation_kernel < 1:
        # Menyimpan nilai ke `dilation_kernel` untuk dipakai pada langkah berikutnya.
        dilation_kernel = 1
    # Mengecek syarat sebelum melanjutkan proses.
    if dilation_kernel % 2 == 0:
        # Menyimpan nilai ke `dilation_kernel +` untuk dipakai pada langkah berikutnya.
        dilation_kernel += 1
    # Mengecek syarat sebelum melanjutkan proses.
    if context_kernel < 1:
        # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
        context_kernel = 1
    # Mengecek syarat sebelum melanjutkan proses.
    if context_kernel % 2 == 0:
        # Menyimpan nilai ke `context_kernel +` untuk dipakai pada langkah berikutnya.
        context_kernel += 1

    # Membuka file/gambar dengan aman lalu menutupnya otomatis setelah selesai.
    with Image.open(path) as image:
        # Mengubah citra ke warna RGB agar format gambar seragam.
        rgb = image.convert("RGB")
        # Mengubah warna RGB ke HSV agar warna merah hotspot lebih mudah dipisahkan.
        hsv = np.asarray(rgb.convert("HSV"), dtype=np.uint8)

    # Mengambil komponen warna H (hue) untuk mengenali warna merah.
    h = hsv[:, :, 0]
    # Mengambil komponen S (saturation) untuk memastikan warna cukup kuat.
    s = hsv[:, :, 1]
    # Mengambil komponen V (brightness) untuk memastikan piksel cukup terang.
    v = hsv[:, :, 2]

    # Menandai piksel merah pada rentang warna merah bagian bawah.
    red_low = (h <= 14) & (s >= 70) & (v >= 50)
    # Menandai piksel merah pada rentang warna merah bagian atas.
    red_high = (h >= 242) & (s >= 70) & (v >= 50)
    # Menggabungkan hasil deteksi merah menjadi mask hotspot.
    mask = ((red_low | red_high).astype(np.uint8)) * 255

    # Mengubah mask angka menjadi gambar hitam-putih.
    mask_image = Image.fromarray(mask)
    # Mengecek syarat sebelum melanjutkan proses.
    if dilation_kernel > 1:
        # Memperbesar titik hotspot kecil agar tidak mudah hilang.
        mask_image = mask_image.filter(ImageFilter.MaxFilter(size=dilation_kernel))

    # Mengecek syarat sebelum melanjutkan proses.
    if mask_image.size != (img_size, img_size):
        # Catatan asli dari pembuat kode.
        # Resize dilakukan setelah mask hotspot dibuat, supaya hotspot kecil
        # Catatan asli dari pembuat kode.
        # tidak hilang akibat interpolasi RGB sebelum thresholding.
        # Menyimpan nilai ke `mask_image` untuk dipakai pada langkah berikutnya.
        mask_image = mask_image.resize((img_size, img_size), Image.BILINEAR)

    # Menyimpan nilai ke `density` untuk dipakai pada langkah berikutnya.
    density = np.asarray(mask_image, dtype=np.float32) / 255.0
    # Menyimpan nilai ke `density` untuk dipakai pada langkah berikutnya.
    density = np.clip(density, 0.0, 1.0)

    # Mengecek syarat sebelum melanjutkan proses.
    if feature_stack == "mask_context3":
        # Menyimpan nilai ke `context_image` untuk dipakai pada langkah berikutnya.
        context_image = Image.fromarray(np.uint8(density * 255.0))
        # Mengecek syarat sebelum melanjutkan proses.
        if context_kernel > 1:
            # Menyimpan nilai ke `blur_radius` untuk dipakai pada langkah berikutnya.
            blur_radius = max(1, context_kernel // 2)
            # Menyimpan nilai ke `context_image` untuk dipakai pada langkah berikutnya.
            context_image = context_image.filter(ImageFilter.BoxBlur(radius=blur_radius))
        # Menyimpan nilai ke `context_map` untuk dipakai pada langkah berikutnya.
        context_map = np.asarray(context_image, dtype=np.float32) / 255.0
        # Menyimpan nilai ke `context_map` untuk dipakai pada langkah berikutnya.
        context_map = np.clip(context_map, 0.0, 1.0)

        # Menyimpan nilai ke `hotspot_core` untuk dipakai pada langkah berikutnya.
        hotspot_core = (density >= 0.25).astype(np.float32)
        # Menyimpan nilai ke `stacked` untuk dipakai pada langkah berikutnya.
        stacked = np.stack([density, context_map, hotspot_core], axis=-1).astype(np.float32)
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return stacked

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return np.expand_dims(density, axis=-1)


# Membuat langkah kerja bernama `load_records`.
def load_records(dataset_dir: Path, image_extensions: tuple[str, ...]) -> list[DatasetRecord]:
    # Menyimpan nilai ke `records` untuk dipakai pada langkah berikutnya.
    records: list[DatasetRecord] = []
    # Menyimpan nilai ke `allowed_extensions` untuk dipakai pada langkah berikutnya.
    allowed_extensions = set(image_extensions)
    # Mengulang proses untuk setiap data dalam daftar.
    for path in sorted(item for item in dataset_dir.iterdir() if item.is_file()):
        # Mengecek syarat sebelum melanjutkan proses.
        if path.suffix.lower() not in allowed_extensions:
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            continue
        # Menyimpan nilai ke `match` untuk dipakai pada langkah berikutnya.
        match = DATE_PATTERN.search(path.name)
        # Mengecek syarat sebelum melanjutkan proses.
        if not match:
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            continue
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        records.append(
            # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
            DatasetRecord(
                # Menyimpan nilai ke `path` untuk dipakai pada langkah berikutnya.
                path=path,
                # Menyimpan nilai ke `date` untuk dipakai pada langkah berikutnya.
                date=datetime.strptime(match.group(1), "%Y-%m-%d"),
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            )
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return sorted(records, key=lambda item: item.date)


# Membuat langkah kerja bernama `validate_records`.
def validate_records(records: list[DatasetRecord]) -> tuple[list[DatasetRecord], list[dict[str, str]]]:
    # Menyimpan nilai ke `valid_records` untuk dipakai pada langkah berikutnya.
    valid_records: list[DatasetRecord] = []
    # Menyimpan nilai ke `skipped_records` untuk dipakai pada langkah berikutnya.
    skipped_records: list[dict[str, str]] = []

    # Mengulang proses untuk setiap data dalam daftar.
    for record in records:
        # Mencoba menjalankan proses yang mungkin gagal.
        try:
            # Mengecek syarat sebelum melanjutkan proses.
            if not record.path.exists():
                # Menghentikan proses dan memberi pesan kesalahan yang jelas.
                raise FileNotFoundError(f"File tidak ditemukan: {record.path}")

            # Membuka file/gambar dengan aman lalu menutupnya otomatis setelah selesai.
            with Image.open(record.path) as image:
                # Melanjutkan langkah kerja pada bagian kode ini.
                image.verify()

            # Melanjutkan langkah kerja pada bagian kode ini.
            valid_records.append(record)
        # Menangani kesalahan agar program tidak langsung berhenti.
        except Exception as exc:
            # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
            skipped_records.append(
                # Memulai susunan data atau perintah yang ditulis beberapa baris.
                {
                    # Melanjutkan langkah kerja pada bagian kode ini.
                    "path": str(record.path),
                    # Melanjutkan langkah kerja pada bagian kode ini.
                    "date": record.date.strftime("%Y-%m-%d"),
                    # Melanjutkan langkah kerja pada bagian kode ini.
                    "reason": str(exc),
                # Menutup susunan data atau perintah yang dimulai sebelumnya.
                }
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            )

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return valid_records, skipped_records


# Membuat langkah kerja bernama `build_sequences`.
def build_sequences(
    # Menjelaskan data `input_images` yang disimpan atau dikirim pada bagian ini.
    input_images: np.ndarray,
    # Menjelaskan data `target_images` yang disimpan atau dikirim pada bagian ini.
    target_images: np.ndarray,
    # Menjelaskan data `seq_length` yang disimpan atau dikirim pada bagian ini.
    seq_length: int,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> tuple[np.ndarray, np.ndarray]:
    # Menyimpan nilai ke `x_data` untuk dipakai pada langkah berikutnya.
    x_data: list[np.ndarray] = []
    # Menyimpan nilai ke `y_data` untuk dipakai pada langkah berikutnya.
    y_data: list[np.ndarray] = []
    # Mengulang proses untuk setiap data dalam daftar.
    for index in range(len(input_images) - seq_length):
        # Melanjutkan langkah kerja pada bagian kode ini.
        x_data.append(input_images[index : index + seq_length])
        # Melanjutkan langkah kerja pada bagian kode ini.
        y_data.append(target_images[index + seq_length])
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return np.asarray(x_data, dtype=np.float32), np.asarray(y_data, dtype=np.float32)


# Membuat langkah kerja bernama `estimate_positive_weight`.
def estimate_positive_weight(y_train: np.ndarray, max_pos_weight: float) -> tuple[float, float]:
    # Menyimpan nilai ke `positive_ratio` untuk dipakai pada langkah berikutnya.
    positive_ratio = float(np.mean(y_train))
    # Menyimpan nilai ke `positive_ratio` untuk dipakai pada langkah berikutnya.
    positive_ratio = max(positive_ratio, 1e-6)
    # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
    pos_weight = min((1.0 - positive_ratio) / positive_ratio, max_pos_weight)
    # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
    pos_weight = max(pos_weight, 1.0)
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return float(pos_weight), float(positive_ratio)


# Membuat langkah kerja bernama `make_weighted_bce_tversky_loss`.
def make_weighted_bce_tversky_loss(pos_weight: float):
    # Menyimpan nilai ke `pos_weight_tensor` untuk dipakai pada langkah berikutnya.
    pos_weight_tensor = tf.constant(pos_weight, dtype=tf.float32)
    # Menyimpan nilai ke `epsilon` untuk dipakai pada langkah berikutnya.
    epsilon = tf.constant(1e-7, dtype=tf.float32)
    # Menyimpan nilai ke `alpha` untuk dipakai pada langkah berikutnya.
    alpha = tf.constant(0.3, dtype=tf.float32)
    # Menyimpan nilai ke `beta` untuk dipakai pada langkah berikutnya.
    beta = tf.constant(0.7, dtype=tf.float32)
    # Menyimpan nilai ke `gamma` untuk dipakai pada langkah berikutnya.
    gamma = tf.constant(0.75, dtype=tf.float32)

    # Membuat langkah kerja bernama `loss`.
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true = tf.cast(y_true, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.cast(y_pred, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.clip_by_value(y_pred, epsilon, 1.0 - epsilon)

        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = -(
            # Melanjutkan langkah kerja pada bagian kode ini.
            (pos_weight_tensor * y_true * tf.math.log(y_pred))
            # Melanjutkan langkah kerja pada bagian kode ini.
            + ((1.0 - y_true) * tf.math.log(1.0 - y_pred))
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = tf.reduce_mean(weighted_bce)

        # Menyimpan nilai ke `intersection` untuk dipakai pada langkah berikutnya.
        intersection = tf.reduce_sum(y_true * y_pred)
        # Menyimpan nilai ke `false_positive` untuk dipakai pada langkah berikutnya.
        false_positive = tf.reduce_sum((1.0 - y_true) * y_pred)
        # Menyimpan nilai ke `false_negative` untuk dipakai pada langkah berikutnya.
        false_negative = tf.reduce_sum(y_true * (1.0 - y_pred))
        # Menyimpan nilai ke `tversky_index` untuk dipakai pada langkah berikutnya.
        tversky_index = (intersection + 1.0) / (
            # Melanjutkan langkah kerja pada bagian kode ini.
            intersection + (alpha * false_positive) + (beta * false_negative) + 1.0
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `focal_tversky_loss` untuk dipakai pada langkah berikutnya.
        focal_tversky_loss = tf.pow(1.0 - tversky_index, gamma)

        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (0.6 * weighted_bce) + (0.4 * focal_tversky_loss)

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return loss


# Membuat langkah kerja bernama `make_weighted_bce_dice_loss`.
def make_weighted_bce_dice_loss(pos_weight: float):
    # Menyimpan nilai ke `pos_weight_tensor` untuk dipakai pada langkah berikutnya.
    pos_weight_tensor = tf.constant(pos_weight, dtype=tf.float32)
    # Menyimpan nilai ke `epsilon` untuk dipakai pada langkah berikutnya.
    epsilon = tf.constant(1e-7, dtype=tf.float32)

    # Membuat langkah kerja bernama `loss`.
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true = tf.cast(y_true, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.cast(y_pred, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.clip_by_value(y_pred, epsilon, 1.0 - epsilon)

        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = -(
            # Melanjutkan langkah kerja pada bagian kode ini.
            (pos_weight_tensor * y_true * tf.math.log(y_pred))
            # Melanjutkan langkah kerja pada bagian kode ini.
            + ((1.0 - y_true) * tf.math.log(1.0 - y_pred))
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = tf.reduce_mean(weighted_bce)

        # Menyimpan nilai ke `intersection` untuk dipakai pada langkah berikutnya.
        intersection = tf.reduce_sum(y_true * y_pred)
        # Menyimpan nilai ke `denominator` untuk dipakai pada langkah berikutnya.
        denominator = tf.reduce_sum(y_true) + tf.reduce_sum(y_pred)
        # Menyimpan nilai ke `dice_loss` untuk dipakai pada langkah berikutnya.
        dice_loss = 1.0 - ((2.0 * intersection + 1.0) / (denominator + 1.0))

        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (0.7 * weighted_bce) + (0.3 * dice_loss)

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return loss


# Membuat langkah kerja bernama `make_weighted_bce_dice_context_loss`.
def make_weighted_bce_dice_context_loss(pos_weight: float, context_kernel: int):
    # Menyimpan nilai ke `pos_weight_tensor` untuk dipakai pada langkah berikutnya.
    pos_weight_tensor = tf.constant(pos_weight, dtype=tf.float32)
    # Menyimpan nilai ke `epsilon` untuk dipakai pada langkah berikutnya.
    epsilon = tf.constant(1e-7, dtype=tf.float32)
    # Mengecek syarat sebelum melanjutkan proses.
    if context_kernel < 1:
        # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
        context_kernel = 1
    # Mengecek syarat sebelum melanjutkan proses.
    if context_kernel % 2 == 0:
        # Menyimpan nilai ke `context_kernel +` untuk dipakai pada langkah berikutnya.
        context_kernel += 1

    # Membuat langkah kerja bernama `soft_dice_score`.
    def soft_dice_score(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        # Menyimpan nilai ke `axes` untuk dipakai pada langkah berikutnya.
        axes = (1, 2, 3)
        # Menyimpan nilai ke `intersection` untuk dipakai pada langkah berikutnya.
        intersection = tf.reduce_sum(y_true * y_pred, axis=axes)
        # Menyimpan nilai ke `denominator` untuk dipakai pada langkah berikutnya.
        denominator = tf.reduce_sum(y_true + y_pred, axis=axes)
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (2.0 * intersection + 1.0) / (denominator + 1.0)

    # Membuat langkah kerja bernama `loss`.
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true = tf.cast(y_true, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.cast(y_pred, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.clip_by_value(y_pred, epsilon, 1.0 - epsilon)

        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = -(
            # Melanjutkan langkah kerja pada bagian kode ini.
            (pos_weight_tensor * y_true * tf.math.log(y_pred))
            # Melanjutkan langkah kerja pada bagian kode ini.
            + ((1.0 - y_true) * tf.math.log(1.0 - y_pred))
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = tf.reduce_mean(weighted_bce)

        # Menyimpan nilai ke `core_dice_loss` untuk dipakai pada langkah berikutnya.
        core_dice_loss = 1.0 - tf.reduce_mean(soft_dice_score(y_true, y_pred))

        # Menyimpan nilai ke `context_true` untuk dipakai pada langkah berikutnya.
        context_true = tf.nn.max_pool2d(
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            y_true,
            # Menyimpan nilai ke `ksize` untuk dipakai pada langkah berikutnya.
            ksize=context_kernel,
            # Menyimpan nilai ke `strides` untuk dipakai pada langkah berikutnya.
            strides=1,
            # Menyimpan nilai ke `padding` untuk dipakai pada langkah berikutnya.
            padding="SAME",
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `context_dice_loss` untuk dipakai pada langkah berikutnya.
        context_dice_loss = 1.0 - tf.reduce_mean(soft_dice_score(context_true, y_pred))

        # Menyimpan nilai ke `false_negative_focus` untuk dipakai pada langkah berikutnya.
        false_negative_focus = tf.reduce_mean(y_true * tf.square(1.0 - y_pred))

        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (
            # Melanjutkan langkah kerja pada bagian kode ini.
            (0.45 * weighted_bce)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.30 * core_dice_loss)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.15 * context_dice_loss)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.10 * false_negative_focus)
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return loss


# Membuat langkah kerja bernama `make_weighted_bce_dice_ohem_loss`.
def make_weighted_bce_dice_ohem_loss(
    # Menjelaskan data `pos_weight` yang disimpan atau dikirim pada bagian ini.
    pos_weight: float,
    # Menjelaskan data `context_kernel` yang disimpan atau dikirim pada bagian ini.
    context_kernel: int,
    # Menjelaskan data `ohem_negative_ratio` yang disimpan atau dikirim pada bagian ini.
    ohem_negative_ratio: float,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
):
    # Menyimpan nilai ke `pos_weight_tensor` untuk dipakai pada langkah berikutnya.
    pos_weight_tensor = tf.constant(pos_weight, dtype=tf.float32)
    # Menyimpan nilai ke `epsilon` untuk dipakai pada langkah berikutnya.
    epsilon = tf.constant(1e-7, dtype=tf.float32)
    # Mengecek syarat sebelum melanjutkan proses.
    if context_kernel < 1:
        # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
        context_kernel = 1
    # Mengecek syarat sebelum melanjutkan proses.
    if context_kernel % 2 == 0:
        # Menyimpan nilai ke `context_kernel +` untuk dipakai pada langkah berikutnya.
        context_kernel += 1
    # Menyimpan nilai ke `ohem_negative_ratio` untuk dipakai pada langkah berikutnya.
    ohem_negative_ratio = max(float(ohem_negative_ratio), 1.0)

    # Membuat langkah kerja bernama `soft_dice_score`.
    def soft_dice_score(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        # Menyimpan nilai ke `axes` untuk dipakai pada langkah berikutnya.
        axes = (1, 2, 3)
        # Menyimpan nilai ke `intersection` untuk dipakai pada langkah berikutnya.
        intersection = tf.reduce_sum(y_true * y_pred, axis=axes)
        # Menyimpan nilai ke `denominator` untuk dipakai pada langkah berikutnya.
        denominator = tf.reduce_sum(y_true + y_pred, axis=axes)
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (2.0 * intersection + 1.0) / (denominator + 1.0)

    # Membuat langkah kerja bernama `loss`.
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true = tf.cast(y_true, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.cast(y_pred, tf.float32)
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred = tf.clip_by_value(y_pred, epsilon, 1.0 - epsilon)

        # Menyimpan nilai ke `weighted_bce` untuk dipakai pada langkah berikutnya.
        weighted_bce = -(
            # Melanjutkan langkah kerja pada bagian kode ini.
            (pos_weight_tensor * y_true * tf.math.log(y_pred))
            # Melanjutkan langkah kerja pada bagian kode ini.
            + ((1.0 - y_true) * tf.math.log(1.0 - y_pred))
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )

        # Menyimpan nilai ke `pos_mask` untuk dipakai pada langkah berikutnya.
        pos_mask = y_true >= 0.5
        # Menyimpan nilai ke `neg_mask` untuk dipakai pada langkah berikutnya.
        neg_mask = tf.logical_not(pos_mask)

        # Menyimpan nilai ke `pos_losses` untuk dipakai pada langkah berikutnya.
        pos_losses = tf.boolean_mask(weighted_bce, pos_mask)
        # Menyimpan nilai ke `neg_losses` untuk dipakai pada langkah berikutnya.
        neg_losses = tf.boolean_mask(weighted_bce, neg_mask)

        # Menyimpan nilai ke `pos_count` untuk dipakai pada langkah berikutnya.
        pos_count = tf.size(pos_losses)
        # Menyimpan nilai ke `neg_count` untuk dipakai pada langkah berikutnya.
        neg_count = tf.size(neg_losses)

        # Membuat langkah kerja bernama `safe_mean`.
        def safe_mean(values: tf.Tensor) -> tf.Tensor:
            # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
            return tf.cond(
                # Melanjutkan langkah kerja pada bagian kode ini.
                tf.size(values) > 0,
                # Menjelaskan data `lambda` yang disimpan atau dikirim pada bagian ini.
                lambda: tf.reduce_mean(values),
                # Menyimpan nilai ke `lambda` untuk dipakai pada langkah berikutnya.
                lambda: tf.constant(0.0, dtype=tf.float32),
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            )

        # Menyimpan nilai ke `positive_loss` untuk dipakai pada langkah berikutnya.
        positive_loss = safe_mean(pos_losses)

        # Menyimpan nilai ke `desired_neg` untuk dipakai pada langkah berikutnya.
        desired_neg = tf.cast(
            # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
            tf.maximum(
                # Melanjutkan langkah kerja pada bagian kode ini.
                1024.0,
                # Menyimpan nilai ke `tf.cast(pos_count, tf.float32) * tf.constant(ohem_negative_ratio, dtype` untuk dipakai pada langkah berikutnya.
                tf.cast(pos_count, tf.float32) * tf.constant(ohem_negative_ratio, dtype=tf.float32),
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            ),
            # Melanjutkan langkah kerja pada bagian kode ini.
            tf.int32,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `desired_neg` untuk dipakai pada langkah berikutnya.
        desired_neg = tf.maximum(1, tf.minimum(desired_neg, neg_count))

        # Membuat langkah kerja bernama `topk_neg_mean`.
        def topk_neg_mean() -> tf.Tensor:
            # Menyimpan nilai ke `values, _` untuk dipakai pada langkah berikutnya.
            values, _ = tf.math.top_k(neg_losses, k=desired_neg, sorted=False)
            # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
            return tf.reduce_mean(values)

        # Menyimpan nilai ke `negative_loss` untuk dipakai pada langkah berikutnya.
        negative_loss = tf.cond(
            # Melanjutkan langkah kerja pada bagian kode ini.
            neg_count > 0,
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            topk_neg_mean,
            # Menyimpan nilai ke `lambda` untuk dipakai pada langkah berikutnya.
            lambda: tf.constant(0.0, dtype=tf.float32),
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )

        # Menyimpan nilai ke `core_dice_loss` untuk dipakai pada langkah berikutnya.
        core_dice_loss = 1.0 - tf.reduce_mean(soft_dice_score(y_true, y_pred))
        # Menyimpan nilai ke `context_true` untuk dipakai pada langkah berikutnya.
        context_true = tf.nn.max_pool2d(
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            y_true,
            # Menyimpan nilai ke `ksize` untuk dipakai pada langkah berikutnya.
            ksize=context_kernel,
            # Menyimpan nilai ke `strides` untuk dipakai pada langkah berikutnya.
            strides=1,
            # Menyimpan nilai ke `padding` untuk dipakai pada langkah berikutnya.
            padding="SAME",
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `context_dice_loss` untuk dipakai pada langkah berikutnya.
        context_dice_loss = 1.0 - tf.reduce_mean(soft_dice_score(context_true, y_pred))
        # Menyimpan nilai ke `false_negative_focus` untuk dipakai pada langkah berikutnya.
        false_negative_focus = tf.reduce_mean(y_true * tf.square(1.0 - y_pred))

        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (
            # Melanjutkan langkah kerja pada bagian kode ini.
            (0.45 * positive_loss)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.20 * negative_loss)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.20 * core_dice_loss)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.10 * context_dice_loss)
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.05 * false_negative_focus)
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return loss


# Membuat langkah kerja bernama `build_loss`.
def build_loss(loss_strategy: str, pos_weight: float, context_kernel: int, ohem_negative_ratio: float):
    # Mengecek syarat sebelum melanjutkan proses.
    if loss_strategy == "wbce_tversky":
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return make_weighted_bce_tversky_loss(pos_weight=pos_weight)
    # Mengecek syarat sebelum melanjutkan proses.
    if loss_strategy == "wbce_dice_context":
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return make_weighted_bce_dice_context_loss(
            # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
            pos_weight=pos_weight,
            # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
            context_kernel=context_kernel,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
    # Mengecek syarat sebelum melanjutkan proses.
    if loss_strategy == "wbce_dice_ohem":
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return make_weighted_bce_dice_ohem_loss(
            # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
            pos_weight=pos_weight,
            # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
            context_kernel=context_kernel,
            # Menyimpan nilai ke `ohem_negative_ratio` untuk dipakai pada langkah berikutnya.
            ohem_negative_ratio=ohem_negative_ratio,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return make_weighted_bce_dice_loss(pos_weight=pos_weight)


# Membuat langkah kerja bernama `conv_block_2d`.
def conv_block_2d(x: tf.Tensor, filters: int, name: str) -> tf.Tensor:
    # Menyimpan nilai ke `x` untuk dipakai pada langkah berikutnya.
    x = Conv2D(filters=filters, kernel_size=(3, 3), activation="relu", padding="same", name=f"{name}_conv1")(x)
    # Menyimpan nilai ke `x` untuk dipakai pada langkah berikutnya.
    x = BatchNormalization(name=f"{name}_bn1")(x)
    # Menyimpan nilai ke `x` untuk dipakai pada langkah berikutnya.
    x = Conv2D(filters=filters, kernel_size=(3, 3), activation="relu", padding="same", name=f"{name}_conv2")(x)
    # Menyimpan nilai ke `x` untuk dipakai pada langkah berikutnya.
    x = BatchNormalization(name=f"{name}_bn2")(x)
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return x


# Membuat langkah kerja bernama `build_model`.
def build_model(
    # Menjelaskan data `seq_length` yang disimpan atau dikirim pada bagian ini.
    seq_length: int,
    # Menjelaskan data `img_size` yang disimpan atau dikirim pada bagian ini.
    img_size: int,
    # Menjelaskan data `channels` yang disimpan atau dikirim pada bagian ini.
    channels: int,
    # Menjelaskan data `pos_weight` yang disimpan atau dikirim pada bagian ini.
    pos_weight: float,
    # Menjelaskan data `loss_strategy` yang disimpan atau dikirim pada bagian ini.
    loss_strategy: str,
    # Menjelaskan data `context_kernel` yang disimpan atau dikirim pada bagian ini.
    context_kernel: int,
    # Menjelaskan data `ohem_negative_ratio` yang disimpan atau dikirim pada bagian ini.
    ohem_negative_ratio: float,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> tf.keras.Model:
    # Mengecek syarat sebelum melanjutkan proses.
    if img_size <= 128:
        # Menyimpan nilai ke `model` untuk dipakai pada langkah berikutnya.
        model = Sequential(
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            [
                # Menyimpan nilai ke `Input(shape` untuk dipakai pada langkah berikutnya.
                Input(shape=(seq_length, img_size, img_size, channels)),
                # Menyimpan nilai ke `ConvLSTM2D(filters` untuk dipakai pada langkah berikutnya.
                ConvLSTM2D(filters=32, kernel_size=(3, 3), padding="same", return_sequences=True),
                # Melanjutkan langkah kerja pada bagian kode ini.
                BatchNormalization(),
                # Melanjutkan langkah kerja pada bagian kode ini.
                Dropout(0.2),
                # Menyimpan nilai ke `ConvLSTM2D(filters` untuk dipakai pada langkah berikutnya.
                ConvLSTM2D(filters=32, kernel_size=(3, 3), padding="same", return_sequences=False),
                # Melanjutkan langkah kerja pada bagian kode ini.
                BatchNormalization(),
                # Melanjutkan langkah kerja pada bagian kode ini.
                Dropout(0.2),
                # Menyimpan nilai ke `Conv2D(filters` untuk dipakai pada langkah berikutnya.
                Conv2D(filters=16, kernel_size=(3, 3), activation="relu", padding="same"),
                # Menyimpan nilai ke `Conv2D(filters` untuk dipakai pada langkah berikutnya.
                Conv2D(filters=1, kernel_size=(1, 1), activation="sigmoid", padding="same"),
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            ]
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `model._name` untuk dipakai pada langkah berikutnya.
        model._name = "convlstm_hotspot_128"
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        model.compile(
            # Menyimpan nilai ke `optimizer` untuk dipakai pada langkah berikutnya.
            optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3),
            # Menyimpan nilai ke `loss` untuk dipakai pada langkah berikutnya.
            loss=build_loss(
                # Menyimpan nilai ke `loss_strategy` untuk dipakai pada langkah berikutnya.
                loss_strategy=loss_strategy,
                # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
                pos_weight=pos_weight,
                # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
                context_kernel=context_kernel,
                # Menyimpan nilai ke `ohem_negative_ratio` untuk dipakai pada langkah berikutnya.
                ohem_negative_ratio=ohem_negative_ratio,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            ),
            # Menyimpan nilai ke `jit_compile` untuk dipakai pada langkah berikutnya.
            jit_compile=False,
            # Menyimpan nilai ke `metrics` untuk dipakai pada langkah berikutnya.
            metrics=[
                # Menyimpan nilai ke `tf.keras.metrics.BinaryAccuracy(name` untuk dipakai pada langkah berikutnya.
                tf.keras.metrics.BinaryAccuracy(name="accuracy", threshold=0.5),
                # Menyimpan nilai ke `tf.keras.metrics.Precision(name` untuk dipakai pada langkah berikutnya.
                tf.keras.metrics.Precision(name="precision", thresholds=0.5),
                # Menyimpan nilai ke `tf.keras.metrics.Recall(name` untuk dipakai pada langkah berikutnya.
                tf.keras.metrics.Recall(name="recall", thresholds=0.5),
                # Menyimpan nilai ke `tf.keras.metrics.AUC(name` untuk dipakai pada langkah berikutnya.
                tf.keras.metrics.AUC(name="pr_auc", curve="PR"),
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            ],
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return model

    # Menyimpan nilai ke `inputs` untuk dipakai pada langkah berikutnya.
    inputs = Input(shape=(seq_length, img_size, img_size, channels), name="sequence_input")

    # Menyimpan nilai ke `enc1` untuk dipakai pada langkah berikutnya.
    enc1 = TimeDistributed(
        # Menyimpan nilai ke `Conv2D(filters` untuk dipakai pada langkah berikutnya.
        Conv2D(filters=16, kernel_size=(3, 3), activation="relu", padding="same"),
        # Menyimpan nilai ke `name` untuk dipakai pada langkah berikutnya.
        name="td_enc1_conv",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )(inputs)
    # Menyimpan nilai ke `enc1` untuk dipakai pada langkah berikutnya.
    enc1 = TimeDistributed(BatchNormalization(), name="td_enc1_bn")(enc1)
    # Menyimpan nilai ke `pooled1` untuk dipakai pada langkah berikutnya.
    pooled1 = TimeDistributed(MaxPooling2D(pool_size=(2, 2)), name="td_pool1")(enc1)

    # Menyimpan nilai ke `enc2` untuk dipakai pada langkah berikutnya.
    enc2 = TimeDistributed(
        # Menyimpan nilai ke `Conv2D(filters` untuk dipakai pada langkah berikutnya.
        Conv2D(filters=32, kernel_size=(3, 3), activation="relu", padding="same"),
        # Menyimpan nilai ke `name` untuk dipakai pada langkah berikutnya.
        name="td_enc2_conv",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )(pooled1)
    # Menyimpan nilai ke `enc2` untuk dipakai pada langkah berikutnya.
    enc2 = TimeDistributed(BatchNormalization(), name="td_enc2_bn")(enc2)
    # Menyimpan nilai ke `pooled2` untuk dipakai pada langkah berikutnya.
    pooled2 = TimeDistributed(MaxPooling2D(pool_size=(2, 2)), name="td_pool2")(enc2)

    # Menyimpan nilai ke `temporal` untuk dipakai pada langkah berikutnya.
    temporal = ConvLSTM2D(
        # Menyimpan nilai ke `filters` untuk dipakai pada langkah berikutnya.
        filters=32,
        # Menyimpan nilai ke `kernel_size` untuk dipakai pada langkah berikutnya.
        kernel_size=(3, 3),
        # Menyimpan nilai ke `padding` untuk dipakai pada langkah berikutnya.
        padding="same",
        # Menyimpan nilai ke `return_sequences` untuk dipakai pada langkah berikutnya.
        return_sequences=True,
        # Menyimpan nilai ke `name` untuk dipakai pada langkah berikutnya.
        name="convlstm_1",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )(pooled2)
    # Menyimpan nilai ke `temporal` untuk dipakai pada langkah berikutnya.
    temporal = BatchNormalization(name="convlstm_1_bn")(temporal)
    # Menyimpan nilai ke `temporal` untuk dipakai pada langkah berikutnya.
    temporal = Dropout(0.2, name="convlstm_1_dropout")(temporal)

    # Menyimpan nilai ke `temporal` untuk dipakai pada langkah berikutnya.
    temporal = ConvLSTM2D(
        # Menyimpan nilai ke `filters` untuk dipakai pada langkah berikutnya.
        filters=32,
        # Menyimpan nilai ke `kernel_size` untuk dipakai pada langkah berikutnya.
        kernel_size=(3, 3),
        # Menyimpan nilai ke `padding` untuk dipakai pada langkah berikutnya.
        padding="same",
        # Menyimpan nilai ke `return_sequences` untuk dipakai pada langkah berikutnya.
        return_sequences=False,
        # Menyimpan nilai ke `name` untuk dipakai pada langkah berikutnya.
        name="convlstm_2",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )(temporal)
    # Menyimpan nilai ke `temporal` untuk dipakai pada langkah berikutnya.
    temporal = BatchNormalization(name="convlstm_2_bn")(temporal)
    # Menyimpan nilai ke `temporal` untuk dipakai pada langkah berikutnya.
    temporal = Dropout(0.2, name="convlstm_2_dropout")(temporal)

    # Menyimpan nilai ke `skip2` untuk dipakai pada langkah berikutnya.
    skip2 = Lambda(lambda t: t[:, -1], name="skip2_last_frame")(enc2)
    # Menyimpan nilai ke `skip1` untuk dipakai pada langkah berikutnya.
    skip1 = Lambda(lambda t: t[:, -1], name="skip1_last_frame")(enc1)

    # Menyimpan nilai ke `decoder` untuk dipakai pada langkah berikutnya.
    decoder = UpSampling2D(size=(2, 2), interpolation="bilinear", name="up_1")(temporal)
    # Menyimpan nilai ke `decoder` untuk dipakai pada langkah berikutnya.
    decoder = Concatenate(name="concat_skip2")([decoder, skip2])
    # Menyimpan nilai ke `decoder` untuk dipakai pada langkah berikutnya.
    decoder = conv_block_2d(decoder, filters=32, name="decoder_stage1")

    # Menyimpan nilai ke `decoder` untuk dipakai pada langkah berikutnya.
    decoder = UpSampling2D(size=(2, 2), interpolation="bilinear", name="up_2")(decoder)
    # Menyimpan nilai ke `decoder` untuk dipakai pada langkah berikutnya.
    decoder = Concatenate(name="concat_skip1")([decoder, skip1])
    # Menyimpan nilai ke `decoder` untuk dipakai pada langkah berikutnya.
    decoder = conv_block_2d(decoder, filters=16, name="decoder_stage2")

    # Menyimpan nilai ke `outputs` untuk dipakai pada langkah berikutnya.
    outputs = Conv2D(
        # Menyimpan nilai ke `filters` untuk dipakai pada langkah berikutnya.
        filters=1,
        # Menyimpan nilai ke `kernel_size` untuk dipakai pada langkah berikutnya.
        kernel_size=(1, 1),
        # Menyimpan nilai ke `activation` untuk dipakai pada langkah berikutnya.
        activation="sigmoid",
        # Menyimpan nilai ke `padding` untuk dipakai pada langkah berikutnya.
        padding="same",
        # Menyimpan nilai ke `name` untuk dipakai pada langkah berikutnya.
        name="prediction_head",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )(decoder)

    # Menyimpan nilai ke `model` untuk dipakai pada langkah berikutnya.
    model = Model(inputs=inputs, outputs=outputs, name="convlstm_hotspot_256")
    # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
    model.compile(
        # Menyimpan nilai ke `optimizer` untuk dipakai pada langkah berikutnya.
        optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3),
        # Menyimpan nilai ke `loss` untuk dipakai pada langkah berikutnya.
        loss=build_loss(
            # Menyimpan nilai ke `loss_strategy` untuk dipakai pada langkah berikutnya.
            loss_strategy=loss_strategy,
            # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
            pos_weight=pos_weight,
            # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
            context_kernel=context_kernel,
            # Menyimpan nilai ke `ohem_negative_ratio` untuk dipakai pada langkah berikutnya.
            ohem_negative_ratio=ohem_negative_ratio,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Menyimpan nilai ke `jit_compile` untuk dipakai pada langkah berikutnya.
        jit_compile=False,
        # Menyimpan nilai ke `metrics` untuk dipakai pada langkah berikutnya.
        metrics=[
            # Menyimpan nilai ke `tf.keras.metrics.BinaryAccuracy(name` untuk dipakai pada langkah berikutnya.
            tf.keras.metrics.BinaryAccuracy(name="accuracy", threshold=0.5),
            # Menyimpan nilai ke `tf.keras.metrics.Precision(name` untuk dipakai pada langkah berikutnya.
            tf.keras.metrics.Precision(name="precision", thresholds=0.5),
            # Menyimpan nilai ke `tf.keras.metrics.Recall(name` untuk dipakai pada langkah berikutnya.
            tf.keras.metrics.Recall(name="recall", thresholds=0.5),
            # Menyimpan nilai ke `tf.keras.metrics.AUC(name` untuk dipakai pada langkah berikutnya.
            tf.keras.metrics.AUC(name="pr_auc", curve="PR"),
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ],
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return model


# Membuat langkah kerja bernama `build_datasets`.
def build_datasets(
    # Menjelaskan data `x_train` yang disimpan atau dikirim pada bagian ini.
    x_train: np.ndarray,
    # Menjelaskan data `y_train` yang disimpan atau dikirim pada bagian ini.
    y_train: np.ndarray,
    # Menjelaskan data `x_val` yang disimpan atau dikirim pada bagian ini.
    x_val: np.ndarray,
    # Menjelaskan data `y_val` yang disimpan atau dikirim pada bagian ini.
    y_val: np.ndarray,
    # Menjelaskan data `batch_size` yang disimpan atau dikirim pada bagian ini.
    batch_size: int,
    # Menjelaskan data `enable_augmentation` yang disimpan atau dikirim pada bagian ini.
    enable_augmentation: bool,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> tuple[tf.data.Dataset, tf.data.Dataset]:
    # Menyimpan nilai ke `autotune` untuk dipakai pada langkah berikutnya.
    autotune = tf.data.AUTOTUNE

    # Menyimpan nilai ke `train_ds` untuk dipakai pada langkah berikutnya.
    train_ds = tf.data.Dataset.from_tensor_slices((x_train, y_train))
    # Menyimpan nilai ke `train_ds` untuk dipakai pada langkah berikutnya.
    train_ds = train_ds.shuffle(buffer_size=min(len(x_train), 256), reshuffle_each_iteration=True)

    # Mengecek syarat sebelum melanjutkan proses.
    if enable_augmentation:

        # Membuat langkah kerja bernama `augment`.
        def augment(sequence: tf.Tensor, target: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
            # Menyimpan nilai ke `stacked` untuk dipakai pada langkah berikutnya.
            stacked = tf.concat([sequence, tf.expand_dims(target, axis=0)], axis=0)

            # Mengecek syarat sebelum melanjutkan proses.
            if tf.random.uniform(()) > 0.5:
                # Menyimpan nilai ke `stacked` untuk dipakai pada langkah berikutnya.
                stacked = tf.image.flip_left_right(stacked)
            # Mengecek syarat sebelum melanjutkan proses.
            if tf.random.uniform(()) > 0.5:
                # Menyimpan nilai ke `stacked` untuk dipakai pada langkah berikutnya.
                stacked = tf.image.flip_up_down(stacked)

            # Menyimpan nilai ke `k` untuk dipakai pada langkah berikutnya.
            k = tf.random.uniform(shape=(), minval=0, maxval=4, dtype=tf.int32)
            # Menyimpan nilai ke `stacked` untuk dipakai pada langkah berikutnya.
            stacked = tf.image.rot90(stacked, k=k)

            # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
            return stacked[:-1], stacked[-1]

        # Menyimpan nilai ke `train_ds` untuk dipakai pada langkah berikutnya.
        train_ds = train_ds.map(augment, num_parallel_calls=autotune)

    # Menyimpan nilai ke `train_ds` untuk dipakai pada langkah berikutnya.
    train_ds = train_ds.batch(batch_size).prefetch(autotune)

    # Menyimpan nilai ke `val_ds` untuk dipakai pada langkah berikutnya.
    val_ds = tf.data.Dataset.from_tensor_slices((x_val, y_val))
    # Menyimpan nilai ke `val_ds` untuk dipakai pada langkah berikutnya.
    val_ds = val_ds.batch(batch_size).prefetch(autotune)
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return train_ds, val_ds


# Membuat langkah kerja bernama `oversample_positive_sequences`.
def oversample_positive_sequences(
    # Menjelaskan data `x_train` yang disimpan atau dikirim pada bagian ini.
    x_train: np.ndarray,
    # Menjelaskan data `y_train` yang disimpan atau dikirim pada bagian ini.
    y_train: np.ndarray,
    # Menjelaskan data `ground_truth_threshold` yang disimpan atau dikirim pada bagian ini.
    ground_truth_threshold: float,
    # Menjelaskan data `positive_oversample_factor` yang disimpan atau dikirim pada bagian ini.
    positive_oversample_factor: int,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> tuple[np.ndarray, np.ndarray, dict[str, int]]:
    # Menyimpan nilai ke `positive_mask` untuk dipakai pada langkah berikutnya.
    positive_mask = np.any(y_train >= ground_truth_threshold, axis=(1, 2, 3))
    # Menyimpan nilai ke `positive_indices` untuk dipakai pada langkah berikutnya.
    positive_indices = np.where(positive_mask)[0]
    # Menyimpan nilai ke `negative_indices` untuk dipakai pada langkah berikutnya.
    negative_indices = np.where(np.logical_not(positive_mask))[0]

    # Menyimpan nilai ke `stats` untuk dipakai pada langkah berikutnya.
    stats = {
        # Melanjutkan langkah kerja pada bagian kode ini.
        "positive_sequence_count_before": int(len(positive_indices)),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "negative_sequence_count_before": int(len(negative_indices)),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "oversample_factor": int(max(1, positive_oversample_factor)),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    }

    # Mengecek syarat sebelum melanjutkan proses.
    if positive_oversample_factor <= 1 or len(positive_indices) == 0:
        # Menyimpan nilai ke `stats["train_sequence_count_after"]` untuk dipakai pada langkah berikutnya.
        stats["train_sequence_count_after"] = int(len(x_train))
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return x_train, y_train, stats

    # Menyimpan nilai ke `extra_x` untuk dipakai pada langkah berikutnya.
    extra_x = [x_train]
    # Menyimpan nilai ke `extra_y` untuk dipakai pada langkah berikutnya.
    extra_y = [y_train]
    # Mengulang proses untuk setiap data dalam daftar.
    for _ in range(positive_oversample_factor - 1):
        # Melanjutkan langkah kerja pada bagian kode ini.
        extra_x.append(x_train[positive_indices])
        # Melanjutkan langkah kerja pada bagian kode ini.
        extra_y.append(y_train[positive_indices])

    # Menyimpan nilai ke `combined_x` untuk dipakai pada langkah berikutnya.
    combined_x = np.concatenate(extra_x, axis=0)
    # Menyimpan nilai ke `combined_y` untuk dipakai pada langkah berikutnya.
    combined_y = np.concatenate(extra_y, axis=0)

    # Menyimpan nilai ke `permutation` untuk dipakai pada langkah berikutnya.
    permutation = np.random.permutation(len(combined_x))
    # Menyimpan nilai ke `combined_x` untuk dipakai pada langkah berikutnya.
    combined_x = combined_x[permutation]
    # Menyimpan nilai ke `combined_y` untuk dipakai pada langkah berikutnya.
    combined_y = combined_y[permutation]

    # Menyimpan nilai ke `stats["train_sequence_count_after"]` untuk dipakai pada langkah berikutnya.
    stats["train_sequence_count_after"] = int(len(combined_x))
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return combined_x, combined_y, stats


# Membuat langkah kerja bernama `compute_metrics`.
def compute_metrics(
    # Menjelaskan data `y_true` yang disimpan atau dikirim pada bagian ini.
    y_true: np.ndarray,
    # Menjelaskan data `y_pred` yang disimpan atau dikirim pada bagian ini.
    y_pred: np.ndarray,
    # Menjelaskan data `threshold` yang disimpan atau dikirim pada bagian ini.
    threshold: float,
    # Menjelaskan data `ground_truth_threshold` yang disimpan atau dikirim pada bagian ini.
    ground_truth_threshold: float,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> dict[str, float]:
    # Menyimpan nilai ke `truth` untuk dipakai pada langkah berikutnya.
    truth = y_true >= ground_truth_threshold
    # Menyimpan nilai ke `pred` untuk dipakai pada langkah berikutnya.
    pred = y_pred >= threshold

    # Menyimpan nilai ke `tp` untuk dipakai pada langkah berikutnya.
    tp = int(np.logical_and(pred, truth).sum())
    # Menyimpan nilai ke `fp` untuk dipakai pada langkah berikutnya.
    fp = int(np.logical_and(pred, np.logical_not(truth)).sum())
    # Menyimpan nilai ke `fn` untuk dipakai pada langkah berikutnya.
    fn = int(np.logical_and(np.logical_not(pred), truth).sum())
    # Menyimpan nilai ke `tn` untuk dipakai pada langkah berikutnya.
    tn = int(np.logical_and(np.logical_not(pred), np.logical_not(truth)).sum())

    # Membuat langkah kerja bernama `safe_div`.
    def safe_div(a: float, b: float) -> float:
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return 0.0 if b == 0 else float(a / b)

    # Menyimpan nilai ke `precision` untuk dipakai pada langkah berikutnya.
    precision = safe_div(tp, tp + fp)
    # Menyimpan nilai ke `recall` untuk dipakai pada langkah berikutnya.
    recall = safe_div(tp, tp + fn)
    # Menyimpan nilai ke `f1_score` untuk dipakai pada langkah berikutnya.
    f1_score = safe_div(2 * precision * recall, precision + recall)
    # Menyimpan nilai ke `iou` untuk dipakai pada langkah berikutnya.
    iou = safe_div(tp, tp + fp + fn)
    # Menyimpan nilai ke `accuracy` untuk dipakai pada langkah berikutnya.
    accuracy = safe_div(tp + tn, tp + tn + fp + fn)

    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return {
        # Melanjutkan langkah kerja pada bagian kode ini.
        "tp": tp,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "fp": fp,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "fn": fn,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "tn": tn,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "precision": precision,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "recall": recall,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "f1_score": f1_score,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "iou": iou,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "accuracy": accuracy,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    }


# Membuat langkah kerja bernama `sweep_thresholds`.
def sweep_thresholds(
    # Menjelaskan data `y_true` yang disimpan atau dikirim pada bagian ini.
    y_true: np.ndarray,
    # Menjelaskan data `y_pred` yang disimpan atau dikirim pada bagian ini.
    y_pred: np.ndarray,
    # Menjelaskan data `ground_truth_threshold` yang disimpan atau dikirim pada bagian ini.
    ground_truth_threshold: float,
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> tuple[float, list[dict[str, float]], dict[str, float]]:
    # Menyimpan nilai ke `sweep_values` untuk dipakai pada langkah berikutnya.
    sweep_values = [round(value, 2) for value in np.arange(0.05, 1.00, 0.05)]
    # Menyimpan nilai ke `results` untuk dipakai pada langkah berikutnya.
    results: list[dict[str, float]] = []
    # Menyimpan nilai ke `best_threshold` untuk dipakai pada langkah berikutnya.
    best_threshold = 0.5
    # Menyimpan nilai ke `best_metrics` untuk dipakai pada langkah berikutnya.
    best_metrics: dict[str, float] | None = None

    # Mengulang proses untuk setiap data dalam daftar.
    for threshold in sweep_values:
        # Menyimpan nilai ke `metrics` untuk dipakai pada langkah berikutnya.
        metrics = compute_metrics(
            # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
            y_true=y_true,
            # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
            y_pred=y_pred,
            # Menyimpan nilai ke `threshold` untuk dipakai pada langkah berikutnya.
            threshold=threshold,
            # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
            ground_truth_threshold=ground_truth_threshold,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `entry` untuk dipakai pada langkah berikutnya.
        entry = {"threshold": threshold, **metrics}
        # Melanjutkan langkah kerja pada bagian kode ini.
        results.append(entry)

        # Mengecek syarat sebelum melanjutkan proses.
        if best_metrics is None:
            # Menyimpan nilai ke `best_threshold` untuk dipakai pada langkah berikutnya.
            best_threshold = threshold
            # Menyimpan nilai ke `best_metrics` untuk dipakai pada langkah berikutnya.
            best_metrics = metrics
            # Menyebutkan item yang ikut dipakai pada daftar di atas.
            continue

        # Menyimpan nilai ke `candidate_key` untuk dipakai pada langkah berikutnya.
        candidate_key = (
            # Melanjutkan langkah kerja pada bagian kode ini.
            metrics["f1_score"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            metrics["iou"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            metrics["precision"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            metrics["recall"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            -metrics["fp"],
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `current_key` untuk dipakai pada langkah berikutnya.
        current_key = (
            # Melanjutkan langkah kerja pada bagian kode ini.
            best_metrics["f1_score"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            best_metrics["iou"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            best_metrics["precision"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            best_metrics["recall"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            -best_metrics["fp"],
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Mengecek syarat sebelum melanjutkan proses.
        if candidate_key > current_key:
            # Menyimpan nilai ke `best_threshold` untuk dipakai pada langkah berikutnya.
            best_threshold = threshold
            # Menyimpan nilai ke `best_metrics` untuk dipakai pada langkah berikutnya.
            best_metrics = metrics

    # Mengecek bahwa hasil test sesuai dengan yang diharapkan.
    assert best_metrics is not None
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return best_threshold, results, best_metrics


# Membuat wadah bernama `ValidationSweepCallback` untuk menyimpan data atau aturan kerja.
class ValidationSweepCallback(tf.keras.callbacks.Callback):
    # Membuat langkah kerja bernama `__init__`.
    def __init__(
        # Menyebutkan item yang ikut dipakai pada daftar di atas.
        self,
        # Menjelaskan data `x_val` yang disimpan atau dikirim pada bagian ini.
        x_val: np.ndarray,
        # Menjelaskan data `y_val` yang disimpan atau dikirim pada bagian ini.
        y_val: np.ndarray,
        # Menjelaskan data `ground_truth_threshold` yang disimpan atau dikirim pada bagian ini.
        ground_truth_threshold: float,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ) -> None:
        # Melanjutkan langkah kerja pada bagian kode ini.
        super().__init__()
        # Menyimpan nilai ke `self.x_val` untuk dipakai pada langkah berikutnya.
        self.x_val = x_val
        # Menyimpan nilai ke `self.y_val` untuk dipakai pada langkah berikutnya.
        self.y_val = y_val
        # Menyimpan nilai ke `self.ground_truth_threshold` untuk dipakai pada langkah berikutnya.
        self.ground_truth_threshold = ground_truth_threshold

    # Membuat langkah kerja bernama `on_epoch_end`.
    def on_epoch_end(self, epoch: int, logs: dict | None = None) -> None:
        # Menyimpan nilai ke `logs` untuk dipakai pada langkah berikutnya.
        logs = logs or {}
        # Menyimpan nilai ke `predictions` untuk dipakai pada langkah berikutnya.
        predictions = self.model.predict(self.x_val, verbose=0)
        # Menyimpan nilai ke `best_threshold, _, best_metrics` untuk dipakai pada langkah berikutnya.
        best_threshold, _, best_metrics = sweep_thresholds(
            # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
            y_true=self.y_val,
            # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
            y_pred=predictions,
            # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
            ground_truth_threshold=self.ground_truth_threshold,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Menyimpan nilai ke `logs["val_best_threshold"]` untuk dipakai pada langkah berikutnya.
        logs["val_best_threshold"] = float(best_threshold)
        # Menyimpan nilai ke `logs["val_best_precision"]` untuk dipakai pada langkah berikutnya.
        logs["val_best_precision"] = float(best_metrics["precision"])
        # Menyimpan nilai ke `logs["val_best_recall"]` untuk dipakai pada langkah berikutnya.
        logs["val_best_recall"] = float(best_metrics["recall"])
        # Menyimpan nilai ke `logs["val_best_f1"]` untuk dipakai pada langkah berikutnya.
        logs["val_best_f1"] = float(best_metrics["f1_score"])
        # Menyimpan nilai ke `logs["val_best_iou"]` untuk dipakai pada langkah berikutnya.
        logs["val_best_iou"] = float(best_metrics["iou"])
        # Menyimpan nilai ke `logs["val_model_score"]` untuk dipakai pada langkah berikutnya.
        logs["val_model_score"] = float(self._compute_model_score(best_metrics))
        # Menampilkan informasi ke terminal agar proses mudah dicek.
        print(
            # Menyimpan nilai ke `f"\nEpoch {epoch + 1}` untuk dipakai pada langkah berikutnya.
            f"\nEpoch {epoch + 1}: val_best_threshold={best_threshold:.2f} "
            # Menyimpan nilai ke `f"val_best_f1` untuk dipakai pada langkah berikutnya.
            f"val_best_f1={best_metrics['f1_score']:.4f} "
            # Menyimpan nilai ke `f"val_best_iou` untuk dipakai pada langkah berikutnya.
            f"val_best_iou={best_metrics['iou']:.4f} "
            # Menyimpan nilai ke `f"val_best_recall` untuk dipakai pada langkah berikutnya.
            f"val_best_recall={best_metrics['recall']:.4f}"
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )

    # Memberi tanda khusus untuk cara kerja fungsi atau class di bawahnya.
    @staticmethod
    # Membuat langkah kerja bernama `_compute_model_score`.
    def _compute_model_score(metrics: dict[str, float]) -> float:
        # Catatan asli dari pembuat kode.
        # Hindari memilih solusi degeneratif yang hampir memprediksi semua piksel
        # Catatan asli dari pembuat kode.
        # sebagai positif hanya karena recall tinggi di threshold sangat rendah.
        # Mengecek syarat sebelum melanjutkan proses.
        if metrics["precision"] < 0.02 or metrics["f1_score"] < 0.02:
            # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
            return 0.0
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return float(
            # Melanjutkan langkah kerja pada bagian kode ini.
            (0.60 * metrics["f1_score"])
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.20 * metrics["iou"])
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.10 * metrics["precision"])
            # Melanjutkan langkah kerja pada bagian kode ini.
            + (0.10 * metrics["recall"])
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )


# Membuat langkah kerja bernama `clone_args`.
def clone_args(base_args: argparse.Namespace, **overrides) -> argparse.Namespace:
    # Menyimpan nilai ke `payload` untuk dipakai pada langkah berikutnya.
    payload = vars(base_args).copy()
    # Melanjutkan langkah kerja pada bagian kode ini.
    payload.update(overrides)
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return argparse.Namespace(**payload)


# Membuat langkah kerja bernama `is_notebook_runtime`.
def is_notebook_runtime() -> bool:
    # Mencoba menjalankan proses yang mungkin gagal.
    try:
        # Mengambil bagian tertentu dari library agar bisa dipakai di file ini.
        from IPython import get_ipython  # type: ignore
    # Menangani kesalahan agar program tidak langsung berhenti.
    except Exception:
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return False

    # Menyimpan nilai ke `shell` untuk dipakai pada langkah berikutnya.
    shell = get_ipython()
    # Mengecek syarat sebelum melanjutkan proses.
    if shell is None:
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return False
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return shell.__class__.__name__ in {"ZMQInteractiveShell", "Shell"}


# Membuat langkah kerja bernama `build_candidate_path`.
def build_candidate_path(base_path: Path, label: str) -> Path:
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return base_path.with_name(f"{base_path.stem}_{label}{base_path.suffix}")


# Membuat langkah kerja bernama `run_single_training`.
def run_single_training(
    # Menjelaskan data `args` yang disimpan atau dikirim pada bagian ini.
    args: argparse.Namespace,
    # Menjelaskan data `output_model` yang disimpan atau dikirim pada bagian ini.
    output_model: Path,
    # Menjelaskan data `output_report` yang disimpan atau dikirim pada bagian ini.
    output_report: Path,
    # Menyimpan nilai ke `experiment_name` untuk dipakai pada langkah berikutnya.
    experiment_name: str = "single",
# Menutup susunan data atau perintah yang dimulai sebelumnya.
) -> dict:
    # Menyimpan nilai ke `dataset_dir` untuk dipakai pada langkah berikutnya.
    dataset_dir = resolve_dataset_dir(args.dataset_dir)
    # Menyimpan nilai ke `image_extensions` untuk dipakai pada langkah berikutnya.
    image_extensions = parse_image_extensions(args.image_extensions)

    # Menyimpan nilai ke `records` untuk dipakai pada langkah berikutnya.
    records = load_records(dataset_dir, image_extensions)
    # Mengecek syarat sebelum melanjutkan proses.
    if len(records) <= args.seq_length:
        # Menghentikan proses dan memberi pesan kesalahan yang jelas.
        raise ValueError("Jumlah data tidak cukup untuk membentuk sequence.")

    # Menyimpan nilai ke `records, skipped_records` untuk dipakai pada langkah berikutnya.
    records, skipped_records = validate_records(records)
    # Mengecek syarat sebelum melanjutkan proses.
    if skipped_records:
        # Menampilkan informasi ke terminal agar proses mudah dicek.
        print(f"File dataset yang dilewati: {len(skipped_records)}")
        # Mengulang proses untuk setiap data dalam daftar.
        for item in skipped_records[:10]:
            # Menampilkan informasi ke terminal agar proses mudah dicek.
            print(f"- skip {item['path']} | alasan: {item['reason']}")

    # Mengecek syarat sebelum melanjutkan proses.
    if len(records) <= args.seq_length:
        # Menghentikan proses dan memberi pesan kesalahan yang jelas.
        raise ValueError("Jumlah data valid tidak cukup untuk membentuk sequence.")

    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Dataset ditemukan: {len(records)} frame")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Rentang data: {records[0].date.date()} s.d. {records[-1].date.date()}")

    # Menyimpan nilai ke `input_images` untuk dipakai pada langkah berikutnya.
    input_images = np.stack(
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        [
            # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
            _extract_hotspot_density(
                # Melanjutkan langkah kerja pada bagian kode ini.
                record.path,
                # Menyimpan nilai ke `img_size` untuk dipakai pada langkah berikutnya.
                img_size=args.img_size,
                # Menyimpan nilai ke `dilation_kernel` untuk dipakai pada langkah berikutnya.
                dilation_kernel=args.dilation_kernel,
                # Menyimpan nilai ke `feature_stack` untuk dipakai pada langkah berikutnya.
                feature_stack=args.feature_stack,
                # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
                context_kernel=args.context_kernel,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            )
            # Mengulang proses untuk setiap data dalam daftar.
            for record in records
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ],
        # Menyimpan nilai ke `axis` untuk dipakai pada langkah berikutnya.
        axis=0,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ).astype(np.float32)

    # Menyimpan nilai ke `target_images` untuk dipakai pada langkah berikutnya.
    target_images = np.stack(
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        [
            # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
            _extract_hotspot_density(
                # Melanjutkan langkah kerja pada bagian kode ini.
                record.path,
                # Menyimpan nilai ke `img_size` untuk dipakai pada langkah berikutnya.
                img_size=args.img_size,
                # Menyimpan nilai ke `dilation_kernel` untuk dipakai pada langkah berikutnya.
                dilation_kernel=args.dilation_kernel,
                # Menyimpan nilai ke `feature_stack` untuk dipakai pada langkah berikutnya.
                feature_stack="mask",
                # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
                context_kernel=args.context_kernel,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            )
            # Mengulang proses untuk setiap data dalam daftar.
            for record in records
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ],
        # Menyimpan nilai ke `axis` untuk dipakai pada langkah berikutnya.
        axis=0,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ).astype(np.float32)

    # Menyimpan nilai ke `x_data, y_data` untuk dipakai pada langkah berikutnya.
    x_data, y_data = build_sequences(
        # Menyimpan nilai ke `input_images` untuk dipakai pada langkah berikutnya.
        input_images=input_images,
        # Menyimpan nilai ke `target_images` untuk dipakai pada langkah berikutnya.
        target_images=target_images,
        # Menyimpan nilai ke `seq_length` untuk dipakai pada langkah berikutnya.
        seq_length=args.seq_length,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `sample_count` untuk dipakai pada langkah berikutnya.
    sample_count = len(x_data)
    # Mengecek syarat sebelum melanjutkan proses.
    if sample_count < 3:
        # Menghentikan proses dan memberi pesan kesalahan yang jelas.
        raise ValueError("Jumlah sample sequence terlalu sedikit.")

    # Menyimpan nilai ke `train_end` untuk dipakai pada langkah berikutnya.
    train_end = max(1, int(sample_count * args.train_ratio))
    # Menyimpan nilai ke `val_end` untuk dipakai pada langkah berikutnya.
    val_end = max(train_end + 1, int(sample_count * (args.train_ratio + args.val_ratio)))
    # Menyimpan nilai ke `val_end` untuk dipakai pada langkah berikutnya.
    val_end = min(val_end, sample_count - 1)

    # Menyimpan nilai ke `x_train, y_train` untuk dipakai pada langkah berikutnya.
    x_train, y_train = x_data[:train_end], y_data[:train_end]
    # Menyimpan nilai ke `x_val, y_val` untuk dipakai pada langkah berikutnya.
    x_val, y_val = x_data[train_end:val_end], y_data[train_end:val_end]
    # Menyimpan nilai ke `x_test, y_test` untuk dipakai pada langkah berikutnya.
    x_test, y_test = x_data[val_end:], y_data[val_end:]

    # Mengecek syarat sebelum melanjutkan proses.
    if len(x_val) == 0 or len(x_test) == 0:
        # Menghentikan proses dan memberi pesan kesalahan yang jelas.
        raise ValueError("Split train/val/test gagal. Periksa rasio dataset.")

    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Train samples: {len(x_train)}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Val samples: {len(x_val)}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Test samples: {len(x_test)}")

    # Menyimpan nilai ke `x_train, y_train, oversample_stats` untuk dipakai pada langkah berikutnya.
    x_train, y_train, oversample_stats = oversample_positive_sequences(
        # Menyimpan nilai ke `x_train` untuk dipakai pada langkah berikutnya.
        x_train=x_train,
        # Menyimpan nilai ke `y_train` untuk dipakai pada langkah berikutnya.
        y_train=y_train,
        # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
        ground_truth_threshold=args.ground_truth_threshold,
        # Menyimpan nilai ke `positive_oversample_factor` untuk dipakai pada langkah berikutnya.
        positive_oversample_factor=args.positive_oversample_factor,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(
        # Melanjutkan langkah kerja pada bagian kode ini.
        f"[{experiment_name}] Positive sequence oversampling:",
        # Melanjutkan langkah kerja pada bagian kode ini.
        oversample_stats["positive_sequence_count_before"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "positif,",
        # Melanjutkan langkah kerja pada bagian kode ini.
        oversample_stats["negative_sequence_count_before"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "negatif,",
        # Menyimpan nilai ke `"total setelah oversampling` untuk dipakai pada langkah berikutnya.
        "total setelah oversampling =",
        # Melanjutkan langkah kerja pada bagian kode ini.
        oversample_stats["train_sequence_count_after"],
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Menyimpan nilai ke `pos_weight, positive_ratio` untuk dipakai pada langkah berikutnya.
    pos_weight, positive_ratio = estimate_positive_weight(
        # Menyimpan nilai ke `y_train` untuk dipakai pada langkah berikutnya.
        y_train=y_train,
        # Menyimpan nilai ke `max_pos_weight` untuk dipakai pada langkah berikutnya.
        max_pos_weight=args.max_pos_weight,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Positive mass ratio (train): {positive_ratio:.8f}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Positive class weight: {pos_weight:.4f}")

    # Menyimpan nilai ke `model` untuk dipakai pada langkah berikutnya.
    model = build_model(
        # Menyimpan nilai ke `seq_length` untuk dipakai pada langkah berikutnya.
        seq_length=args.seq_length,
        # Menyimpan nilai ke `img_size` untuk dipakai pada langkah berikutnya.
        img_size=args.img_size,
        # Menyimpan nilai ke `channels` untuk dipakai pada langkah berikutnya.
        channels=input_images.shape[-1],
        # Menyimpan nilai ke `pos_weight` untuk dipakai pada langkah berikutnya.
        pos_weight=pos_weight,
        # Menyimpan nilai ke `loss_strategy` untuk dipakai pada langkah berikutnya.
        loss_strategy=args.loss_strategy,
        # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
        context_kernel=args.context_kernel,
        # Menyimpan nilai ke `ohem_negative_ratio` untuk dipakai pada langkah berikutnya.
        ohem_negative_ratio=args.ohem_negative_ratio,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `train_dataset, val_dataset` untuk dipakai pada langkah berikutnya.
    train_dataset, val_dataset = build_datasets(
        # Menyimpan nilai ke `x_train` untuk dipakai pada langkah berikutnya.
        x_train=x_train,
        # Menyimpan nilai ke `y_train` untuk dipakai pada langkah berikutnya.
        y_train=y_train,
        # Menyimpan nilai ke `x_val` untuk dipakai pada langkah berikutnya.
        x_val=x_val,
        # Menyimpan nilai ke `y_val` untuk dipakai pada langkah berikutnya.
        y_val=y_val,
        # Menyimpan nilai ke `batch_size` untuk dipakai pada langkah berikutnya.
        batch_size=args.batch_size,
        # Menyimpan nilai ke `enable_augmentation` untuk dipakai pada langkah berikutnya.
        enable_augmentation=args.enable_augmentation,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `output_model.parent.mkdir(parents` untuk dipakai pada langkah berikutnya.
    output_model.parent.mkdir(parents=True, exist_ok=True)
    # Menyimpan nilai ke `output_report.parent.mkdir(parents` untuk dipakai pada langkah berikutnya.
    output_report.parent.mkdir(parents=True, exist_ok=True)

    # Menyimpan nilai ke `callbacks` untuk dipakai pada langkah berikutnya.
    callbacks = [
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        ValidationSweepCallback(
            # Menyimpan nilai ke `x_val` untuk dipakai pada langkah berikutnya.
            x_val=x_val,
            # Menyimpan nilai ke `y_val` untuk dipakai pada langkah berikutnya.
            y_val=y_val,
            # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
            ground_truth_threshold=args.ground_truth_threshold,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        ModelCheckpoint(
            # Menyimpan nilai ke `filepath` untuk dipakai pada langkah berikutnya.
            filepath=str(output_model),
            # Menyimpan nilai ke `monitor` untuk dipakai pada langkah berikutnya.
            monitor="val_model_score",
            # Menyimpan nilai ke `save_best_only` untuk dipakai pada langkah berikutnya.
            save_best_only=True,
            # Menyimpan nilai ke `mode` untuk dipakai pada langkah berikutnya.
            mode="max",
            # Menyimpan nilai ke `verbose` untuk dipakai pada langkah berikutnya.
            verbose=1,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        EarlyStopping(
            # Menyimpan nilai ke `monitor` untuk dipakai pada langkah berikutnya.
            monitor="val_model_score",
            # Menyimpan nilai ke `patience` untuk dipakai pada langkah berikutnya.
            patience=8,
            # Menyimpan nilai ke `restore_best_weights` untuk dipakai pada langkah berikutnya.
            restore_best_weights=True,
            # Menyimpan nilai ke `mode` untuk dipakai pada langkah berikutnya.
            mode="max",
            # Menyimpan nilai ke `verbose` untuk dipakai pada langkah berikutnya.
            verbose=1,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
        ReduceLROnPlateau(
            # Menyimpan nilai ke `monitor` untuk dipakai pada langkah berikutnya.
            monitor="val_loss",
            # Menyimpan nilai ke `factor` untuk dipakai pada langkah berikutnya.
            factor=0.5,
            # Menyimpan nilai ke `patience` untuk dipakai pada langkah berikutnya.
            patience=2,
            # Menyimpan nilai ke `min_lr` untuk dipakai pada langkah berikutnya.
            min_lr=1e-6,
            # Menyimpan nilai ke `verbose` untuk dipakai pada langkah berikutnya.
            verbose=1,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ]

    # Menyimpan nilai ke `history` untuk dipakai pada langkah berikutnya.
    history = model.fit(
        # Menyebutkan item yang ikut dipakai pada daftar di atas.
        train_dataset,
        # Menyimpan nilai ke `validation_data` untuk dipakai pada langkah berikutnya.
        validation_data=val_dataset,
        # Menyimpan nilai ke `epochs` untuk dipakai pada langkah berikutnya.
        epochs=args.epochs,
        # Menyimpan nilai ke `callbacks` untuk dipakai pada langkah berikutnya.
        callbacks=callbacks,
        # Menyimpan nilai ke `verbose` untuk dipakai pada langkah berikutnya.
        verbose=1,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Menyimpan nilai ke `val_predictions` untuk dipakai pada langkah berikutnya.
    val_predictions = model.predict(x_val, verbose=0)
    # Menyimpan nilai ke `best_threshold, val_threshold_sweep, best_val_metrics` untuk dipakai pada langkah berikutnya.
    best_threshold, val_threshold_sweep, best_val_metrics = sweep_thresholds(
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true=y_val,
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred=val_predictions,
        # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
        ground_truth_threshold=args.ground_truth_threshold,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Menyimpan nilai ke `test_predictions` untuk dipakai pada langkah berikutnya.
    test_predictions = model.predict(x_test, verbose=0)
    # Menyimpan nilai ke `default_test_metrics` untuk dipakai pada langkah berikutnya.
    default_test_metrics = compute_metrics(
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true=y_test,
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred=test_predictions,
        # Menyimpan nilai ke `threshold` untuk dipakai pada langkah berikutnya.
        threshold=args.threshold,
        # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
        ground_truth_threshold=args.ground_truth_threshold,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `best_test_metrics` untuk dipakai pada langkah berikutnya.
    best_test_metrics = compute_metrics(
        # Menyimpan nilai ke `y_true` untuk dipakai pada langkah berikutnya.
        y_true=y_test,
        # Menyimpan nilai ke `y_pred` untuk dipakai pada langkah berikutnya.
        y_pred=test_predictions,
        # Menyimpan nilai ke `threshold` untuk dipakai pada langkah berikutnya.
        threshold=best_threshold,
        # Menyimpan nilai ke `ground_truth_threshold` untuk dipakai pada langkah berikutnya.
        ground_truth_threshold=args.ground_truth_threshold,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Menyimpan nilai ke `report` untuk dipakai pada langkah berikutnya.
    report = {
        # Melanjutkan langkah kerja pada bagian kode ini.
        "experiment_name": experiment_name,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "dataset_dir": str(dataset_dir),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "image_extensions": list(image_extensions),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "dataset_frame_count": len(records),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "skipped_frame_count": len(skipped_records),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "skipped_records_preview": skipped_records[:20],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "date_start": records[0].date.strftime("%Y-%m-%d"),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "date_end": records[-1].date.strftime("%Y-%m-%d"),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "img_size": args.img_size,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "channels": int(input_images.shape[-1]),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "feature_stack": args.feature_stack,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "seq_length": args.seq_length,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "epochs_requested": args.epochs,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "batch_size": args.batch_size,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "train_ratio": args.train_ratio,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "val_ratio": args.val_ratio,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "default_threshold": args.threshold,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "ground_truth_threshold": args.ground_truth_threshold,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "selected_threshold": best_threshold,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "dilation_kernel": args.dilation_kernel,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "context_kernel": args.context_kernel,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "loss_strategy": args.loss_strategy,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "ohem_negative_ratio": args.ohem_negative_ratio,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "train_augmentation_enabled": args.enable_augmentation,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "positive_sequence_oversampling": oversample_stats,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "positive_ratio_train": positive_ratio,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "positive_class_weight": pos_weight,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "train_samples": len(x_train),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "val_samples": len(x_val),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "test_samples": len(x_test),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "history": history.history,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "validation_threshold_sweep": val_threshold_sweep,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "validation_model_score": ValidationSweepCallback._compute_model_score(best_val_metrics),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "validation_best_metrics": best_val_metrics,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "test_metrics_default_threshold": default_test_metrics,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "test_metrics_best_threshold": best_test_metrics,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "test_metrics": best_test_metrics,
        # Melanjutkan langkah kerja pada bagian kode ini.
        "output_model": str(output_model),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    }

    # Menyimpan nilai ke `output_report.write_text(json.dumps(report, indent` untuk dipakai pada langkah berikutnya.
    output_report.write_text(json.dumps(report, indent=2), encoding="utf-8")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(json.dumps(report["validation_best_metrics"], indent=2))
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(json.dumps(report["test_metrics_best_threshold"], indent=2))
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Threshold terbaik dari validation: {best_threshold}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Model terbaik tersimpan di: {output_model}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[{experiment_name}] Report training tersimpan di: {output_report}")
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return report


# Membuat langkah kerja bernama `run_baseline_search`.
def run_baseline_search(args: argparse.Namespace, output_model: Path, output_report: Path) -> int:
    # Menyimpan nilai ke `search_base` untuk dipakai pada langkah berikutnya.
    search_base = clone_args(
        # Menyebutkan item yang ikut dipakai pada daftar di atas.
        args,
        # Menyimpan nilai ke `img_size` untuk dipakai pada langkah berikutnya.
        img_size=128,
        # Menyimpan nilai ke `epochs` untuk dipakai pada langkah berikutnya.
        epochs=args.search_epochs,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `candidates` untuk dipakai pada langkah berikutnya.
    candidates = [
        # Melanjutkan langkah kerja pada bagian kode ini.
        ("baseline_dice_gt005_k3", {"loss_strategy": "wbce_dice", "ground_truth_threshold": 0.05, "dilation_kernel": 3, "positive_oversample_factor": 1, "enable_augmentation": False}),
        # Melanjutkan langkah kerja pada bagian kode ini.
        ("baseline_dice_gt003_k3", {"loss_strategy": "wbce_dice", "ground_truth_threshold": 0.03, "dilation_kernel": 3, "positive_oversample_factor": 1, "enable_augmentation": False}),
        # Melanjutkan langkah kerja pada bagian kode ini.
        ("baseline_dice_gt005_k5", {"loss_strategy": "wbce_dice", "ground_truth_threshold": 0.05, "dilation_kernel": 5, "positive_oversample_factor": 1, "enable_augmentation": False}),
        # Melanjutkan langkah kerja pada bagian kode ini.
        ("baseline_dice_gt003_k5", {"loss_strategy": "wbce_dice", "ground_truth_threshold": 0.03, "dilation_kernel": 5, "positive_oversample_factor": 1, "enable_augmentation": False}),
        # Melanjutkan langkah kerja pada bagian kode ini.
        ("dice_gt005_k3_aug", {"loss_strategy": "wbce_dice", "ground_truth_threshold": 0.05, "dilation_kernel": 3, "positive_oversample_factor": 1, "enable_augmentation": True}),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ]

    # Menyimpan nilai ke `reports` untuk dipakai pada langkah berikutnya.
    reports: list[dict] = []
    # Mengulang proses untuk setiap data dalam daftar.
    for label, overrides in candidates:
        # Menyimpan nilai ke `candidate_args` untuk dipakai pada langkah berikutnya.
        candidate_args = clone_args(search_base, **overrides)
        # Menyimpan nilai ke `candidate_model` untuk dipakai pada langkah berikutnya.
        candidate_model = build_candidate_path(output_model, label)
        # Menyimpan nilai ke `candidate_report` untuk dipakai pada langkah berikutnya.
        candidate_report = build_candidate_path(output_report, label)
        # Menampilkan informasi ke terminal agar proses mudah dicek.
        print(f"\n=== Menjalankan kandidat: {label} ===")
        # Menyimpan nilai ke `report` untuk dipakai pada langkah berikutnya.
        report = run_single_training(
            # Menyimpan nilai ke `args` untuk dipakai pada langkah berikutnya.
            args=candidate_args,
            # Menyimpan nilai ke `output_model` untuk dipakai pada langkah berikutnya.
            output_model=candidate_model,
            # Menyimpan nilai ke `output_report` untuk dipakai pada langkah berikutnya.
            output_report=candidate_report,
            # Menyimpan nilai ke `experiment_name` untuk dipakai pada langkah berikutnya.
            experiment_name=label,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Melanjutkan langkah kerja pada bagian kode ini.
        reports.append(report)

    # Menyimpan nilai ke `winner` untuk dipakai pada langkah berikutnya.
    winner = max(
        # Menyebutkan item yang ikut dipakai pada daftar di atas.
        reports,
        # Menyimpan nilai ke `key` untuk dipakai pada langkah berikutnya.
        key=lambda item: (
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["validation_best_metrics"]["f1_score"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["validation_best_metrics"]["iou"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["validation_best_metrics"]["recall"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["test_metrics"]["f1_score"],
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Melanjutkan langkah kerja pada bagian kode ini.
    shutil.copy2(winner["output_model"], output_model)
    # Menyimpan nilai ke `summary` untuk dipakai pada langkah berikutnya.
    summary = {
        # Melanjutkan langkah kerja pada bagian kode ini.
        "mode": "search_baseline_128",
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_experiment_name": winner["experiment_name"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_output_model": str(output_model),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_selected_threshold": winner["selected_threshold"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_validation_best_metrics": winner["validation_best_metrics"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_test_metrics": winner["test_metrics"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "candidates": [
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "experiment_name": item["experiment_name"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": item["loss_strategy"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": item["ground_truth_threshold"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "dilation_kernel": item["dilation_kernel"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "train_augmentation_enabled": item["train_augmentation_enabled"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "positive_sequence_oversampling": item["positive_sequence_oversampling"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "selected_threshold": item["selected_threshold"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "validation_best_metrics": item["validation_best_metrics"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "test_metrics": item["test_metrics"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "candidate_output_model": item["output_model"],
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            }
            # Mengulang proses untuk setiap data dalam daftar.
            for item in reports
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ],
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    }
    # Menyimpan nilai ke `output_report.write_text(json.dumps(summary, indent` untuk dipakai pada langkah berikutnya.
    output_report.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"\nKandidat terbaik: {winner['experiment_name']}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(json.dumps(winner["validation_best_metrics"], indent=2))
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(json.dumps(winner["test_metrics"], indent=2))
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"Model pemenang disalin ke: {output_model}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"Ringkasan search disimpan di: {output_report}")
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return 0


# Membuat langkah kerja bernama `run_improved_search`.
def run_improved_search(args: argparse.Namespace, output_model: Path, output_report: Path) -> int:
    # Menyimpan nilai ke `search_base` untuk dipakai pada langkah berikutnya.
    search_base = clone_args(
        # Menyebutkan item yang ikut dipakai pada daftar di atas.
        args,
        # Menyimpan nilai ke `img_size` untuk dipakai pada langkah berikutnya.
        img_size=128,
        # Menyimpan nilai ke `epochs` untuk dipakai pada langkah berikutnya.
        epochs=args.search_epochs,
        # Menyimpan nilai ke `threshold` untuk dipakai pada langkah berikutnya.
        threshold=0.3,
        # Menyimpan nilai ke `positive_oversample_factor` untuk dipakai pada langkah berikutnya.
        positive_oversample_factor=1,
        # Menyimpan nilai ke `dilation_kernel` untuk dipakai pada langkah berikutnya.
        dilation_kernel=5,
        # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
        context_kernel=5,
        # Menyimpan nilai ke `ohem_negative_ratio` untuk dipakai pada langkah berikutnya.
        ohem_negative_ratio=3.0,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Menyimpan nilai ke `candidates` untuk dipakai pada langkah berikutnya.
    candidates = [
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_context_mask_k5",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_context",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_context_mask_k3",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_context",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "dilation_kernel": 3,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "context_kernel": 3,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_ohem_mask_k5",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_ohem",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "dilation_kernel": 5,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "context_kernel": 5,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ohem_negative_ratio": 3.0,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_ohem_mask_k3",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_ohem",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "dilation_kernel": 3,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "context_kernel": 3,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ohem_negative_ratio": 3.0,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_context_stack3_k5",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_context",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask_context3",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_dice_stack3_k5",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask_context3",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_context_stack3_k5_aug",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_context",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.05,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask_context3",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": True,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
        # Memulai susunan data atau perintah yang ditulis beberapa baris.
        (
            # Melanjutkan langkah kerja pada bagian kode ini.
            "improved_context_mask_k5_gt003",
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": "wbce_dice_context",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": 0.03,
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": "mask",
                # Melanjutkan langkah kerja pada bagian kode ini.
                "enable_augmentation": False,
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            },
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    ]

    # Menyimpan nilai ke `reports` untuk dipakai pada langkah berikutnya.
    reports: list[dict] = []
    # Mengulang proses untuk setiap data dalam daftar.
    for label, overrides in candidates:
        # Menyimpan nilai ke `candidate_args` untuk dipakai pada langkah berikutnya.
        candidate_args = clone_args(search_base, **overrides)
        # Menyimpan nilai ke `candidate_model` untuk dipakai pada langkah berikutnya.
        candidate_model = build_candidate_path(output_model, label)
        # Menyimpan nilai ke `candidate_report` untuk dipakai pada langkah berikutnya.
        candidate_report = build_candidate_path(output_report, label)
        # Menampilkan informasi ke terminal agar proses mudah dicek.
        print(f"\n=== Menjalankan kandidat improved: {label} ===")
        # Menyimpan nilai ke `report` untuk dipakai pada langkah berikutnya.
        report = run_single_training(
            # Menyimpan nilai ke `args` untuk dipakai pada langkah berikutnya.
            args=candidate_args,
            # Menyimpan nilai ke `output_model` untuk dipakai pada langkah berikutnya.
            output_model=candidate_model,
            # Menyimpan nilai ke `output_report` untuk dipakai pada langkah berikutnya.
            output_report=candidate_report,
            # Menyimpan nilai ke `experiment_name` untuk dipakai pada langkah berikutnya.
            experiment_name=label,
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        )
        # Melanjutkan langkah kerja pada bagian kode ini.
        reports.append(report)

    # Menyimpan nilai ke `winner` untuk dipakai pada langkah berikutnya.
    winner = max(
        # Menyebutkan item yang ikut dipakai pada daftar di atas.
        reports,
        # Menyimpan nilai ke `key` untuk dipakai pada langkah berikutnya.
        key=lambda item: (
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["validation_model_score"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["validation_best_metrics"]["f1_score"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["validation_best_metrics"]["iou"],
            # Melanjutkan langkah kerja pada bagian kode ini.
            item["test_metrics"]["f1_score"],
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ),
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Melanjutkan langkah kerja pada bagian kode ini.
    shutil.copy2(winner["output_model"], output_model)
    # Menyimpan nilai ke `summary` untuk dipakai pada langkah berikutnya.
    summary = {
        # Melanjutkan langkah kerja pada bagian kode ini.
        "mode": "search_improved_128",
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_experiment_name": winner["experiment_name"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_output_model": str(output_model),
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_selected_threshold": winner["selected_threshold"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_validation_model_score": winner["validation_model_score"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_validation_best_metrics": winner["validation_best_metrics"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "winner_test_metrics": winner["test_metrics"],
        # Melanjutkan langkah kerja pada bagian kode ini.
        "candidates": [
            # Memulai susunan data atau perintah yang ditulis beberapa baris.
            {
                # Melanjutkan langkah kerja pada bagian kode ini.
                "experiment_name": item["experiment_name"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "loss_strategy": item["loss_strategy"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "feature_stack": item["feature_stack"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "channels": item["channels"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ground_truth_threshold": item["ground_truth_threshold"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "dilation_kernel": item["dilation_kernel"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "context_kernel": item["context_kernel"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "ohem_negative_ratio": item["ohem_negative_ratio"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "train_augmentation_enabled": item["train_augmentation_enabled"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "positive_sequence_oversampling": item["positive_sequence_oversampling"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "selected_threshold": item["selected_threshold"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "validation_model_score": item["validation_model_score"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "validation_best_metrics": item["validation_best_metrics"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "test_metrics": item["test_metrics"],
                # Melanjutkan langkah kerja pada bagian kode ini.
                "candidate_output_model": item["output_model"],
            # Menutup susunan data atau perintah yang dimulai sebelumnya.
            }
            # Mengulang proses untuk setiap data dalam daftar.
            for item in reports
        # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ],
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    }
    # Menyimpan nilai ke `output_report.write_text(json.dumps(summary, indent` untuk dipakai pada langkah berikutnya.
    output_report.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"\nKandidat improved terbaik: {winner['experiment_name']}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(json.dumps(winner["validation_best_metrics"], indent=2))
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(json.dumps(winner["test_metrics"], indent=2))
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"Model pemenang disalin ke: {output_model}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"Ringkasan search disimpan di: {output_report}")
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return 0


# Membuat langkah kerja bernama `main`.
def main() -> int:
    # Menyimpan nilai ke `args` untuk dipakai pada langkah berikutnya.
    args = parse_args()
    # Menyimpan nilai ke `output_model, output_report` untuk dipakai pada langkah berikutnya.
    output_model, output_report = resolve_output_paths(
        # Menyimpan nilai ke `output_model` untuk dipakai pada langkah berikutnya.
        output_model=Path(args.output_model).resolve(),
        # Menyimpan nilai ke `output_report` untuk dipakai pada langkah berikutnya.
        output_report=Path(args.output_report).resolve(),
        # Menyimpan nilai ke `overwrite` untuk dipakai pada langkah berikutnya.
        overwrite=args.overwrite,
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )

    # Mengecek syarat sebelum melanjutkan proses.
    if args.search_baseline_128:
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return run_baseline_search(args=args, output_model=output_model, output_report=output_report)
    # Mengecek syarat sebelum melanjutkan proses.
    if args.search_improved_128:
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return run_improved_search(args=args, output_model=output_model, output_report=output_report)

    # Memulai perintah yang dilanjutkan pada baris berikutnya agar lebih rapi.
    run_single_training(
        # Menyimpan nilai ke `args` untuk dipakai pada langkah berikutnya.
        args=args,
        # Menyimpan nilai ke `output_model` untuk dipakai pada langkah berikutnya.
        output_model=output_model,
        # Menyimpan nilai ke `output_report` untuk dipakai pada langkah berikutnya.
        output_report=output_report,
        # Menyimpan nilai ke `experiment_name` untuk dipakai pada langkah berikutnya.
        experiment_name="single_run",
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return 0


# Mengecek syarat sebelum melanjutkan proses.
if __name__ == "__main__" and not is_notebook_runtime():
    # Menghentikan proses dan memberi pesan kesalahan yang jelas.
    raise SystemExit(main())
