# File anotasi dari `Ipynb/TRAIN_CONVLSTM_NATIVE_PATCH_1528x773_GOOGLE_COLAB.py`.
...
Script training patch native untuk eksperimen model ConvLSTM pada citra ukuran asli.
...
COLAB_OUTPUT_MODEL = Path(
    # Melanjutkan langkah kerja pada bagian kode ini.
    "/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/artifacts-native/best_model_convlstm_native_patch_1528x773.keras"
# Menutup susunan data atau perintah yang dimulai sebelumnya.
)
# Menentukan nilai tetap yang dipakai sebagai pengaturan proses.
COLAB_OUTPUT_REPORT = Path(
    # Melanjutkan langkah kerja pada bagian kode ini.
    "/content/drive/MyDrive/Colab Notebooks/Skripsi_Hotspot_ConvLSTM/artifacts-native/convlstm_native_patch_1528x773_training_report.json"
# Menutup susunan data atau perintah yang dimulai sebelumnya.
)
# Menentukan nilai tetap yang dipakai sebagai pengaturan proses.
LOCAL_OUTPUT_MODEL = LOCAL_ARTIFACTS_DIR / "best_model_convlstm_native_patch_1528x773.keras"
# Menentukan nilai tetap yang dipakai sebagai pengaturan proses.
LOCAL_OUTPUT_REPORT = LOCAL_ARTIFACTS_DIR / "convlstm_native_patch_1528x773_training_report.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Latih ConvLSTM native patch 1528x773 untuk dataset hotspot JPG/JPEG/PNG."
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
    )
...
def load_native_mask(path_str: str, native_width: int, native_height: int, dilation_kernel: int) -> np.ndarray:
...
# Menutup susunan data atau perintah yang dimulai sebelumnya.
    ...
    for start in sample_starts:
        # Menggabungkan hasil deteksi merah menjadi mask hotspot.
        mask = load_native_mask(str(records[start + seq_length].path), native_width, native_height, dilation_kernel)
        # Menyimpan nilai ke `positive_sum +` untuk dipakai pada langkah berikutnya.
...
def make_weighted_bce_dice_context_loss(pos_weight: float, context_kernel: int):
    ...
    base_loss = make_weighted_bce_dice_loss(pos_weight)
    # Menyimpan nilai ke `context_kernel` untuk dipakai pada langkah berikutnya.
    context_kernel = _normalize_kernel(context_kernel)
    # Menyimpan nilai ke `epsilon` untuk dipakai pada langkah berikutnya.
    epsilon = tf.constant(1e-7, dtype=tf.float32)
    ...
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        context_bce = tf.reduce_mean(context_weight * context_bce_map)
        # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
        return (0.75 * base_loss(y_true, y_pred)) + (0.25 * context_bce)
...
def build_loss(loss_strategy: str, pos_weight: float, context_kernel: int):
    # Mengecek syarat sebelum melanjutkan proses.
...
class NativePatchSequence(tf.keras.utils.Sequence):
    ...
    # Menutup susunan data atau perintah yang dimulai sebelumnya.
        ...
        # Menyimpan nilai ke `self.dilation_kernel` untuk dipakai pada langkah berikutnya.
...
def sweep_thresholds(y_true: np.ndarray, y_pred: np.ndarray, ground_truth_threshold: float) -> tuple[float, list[dict[str, float]], dict[str, float]]:
...
class ValidationSweepCallback(tf.keras.callbacks.Callback):
    # Membuat langkah kerja bernama `__init__`.
    def __init__(self, val_sequence: NativePatchSequence, y_val: np.ndarray, ground_truth_threshold: float) -> None:
        # Melanjutkan langkah kerja pada bagian kode ini.
        super().__init__()
    ...
    def on_epoch_end(self, epoch: int, logs: dict | None = None) -> None:
        ...
        best_threshold, _, best_metrics = sweep_thresholds(self.y_val, predictions, self.ground_truth_threshold)
...
def run_training(args: argparse.Namespace, output_model: Path, output_report: Path) -> dict:
    ...
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Dataset ditemukan: {len(records)} frame")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Rentang data: {records[0].date.date()} s.d. {records[-1].date.date()}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Train samples: {len(train_starts)}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Val samples: {len(val_starts)}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Test samples: {len(test_starts)}")
    ...
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Positive mass ratio (train): {positive_ratio:.8f}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Positive class weight: {pos_weight:.4f}")
    ...
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Train patch entries: {len(train_entries)}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Val patch entries: {len(val_entries)}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Test patch entries: {len(test_entries)}")
    ...
    callbacks = [
        # Melanjutkan langkah kerja pada bagian kode ini.
        ValidationSweepCallback(val_sequence, y_val, args.ground_truth_threshold),
    ...
    best_threshold, val_sweep, best_val_metrics = sweep_thresholds(y_val, val_predictions, args.ground_truth_threshold)
    ...
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Threshold terbaik dari validation: {best_threshold}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Model terbaik tersimpan di: {output_model}")
    # Menampilkan informasi ke terminal agar proses mudah dicek.
    print(f"[native_patch] Report training tersimpan di: {output_report}")
    # Mengirim hasil dari langkah kerja ini ke bagian yang memanggilnya.
    return report
